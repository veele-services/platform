...
if (
  !allowProduction &&
  (/production|prod/i.test(appEnv) || /production|prod/i.test(databaseUrl))
) {
  throw new Error(
...
async function inspectAssignmentsDefault(client) {
  ...
  if (!exists)
    return {
      ...
      tenant_id_default: "missing-table",
      ok: false,
    };
  ...
  return {
    ...
    ok: Boolean(
      column && column.is_nullable === "NO" && column.column_default === null,
    ),
  };
}

async function inspectTenantIdDefault(client, tableName) {
  const exists = await tableExists(client, tableName);
  if (!exists) {
    return {
      table_name: tableName,
  ...
  return {
    ...
    tenant_id_nullable: column?.is_nullable ?? "missing-column",
    tenant_id_default: column?.column_default ?? null,
    default_removed: Boolean(column && column.column_default === null),
  };
}
...
async function main() {
  ...
  try {
    ...
    for (const tableName of defaultHardenedTables) {
      defaultReports.push(await inspectTenantIdDefault(client, tableName));
    }
    ...
    const report = {
      ...
      default_hardened_tables: defaultReports,
    ...
    if (args.has("--json")) {
      ...
      console.log(
        `- Sensitive unresolved tenant_id rows: \`${report.unresolved_count}\``,
      );
      console.log("\n## Assignments Default\n");
      console.log(
        markdownTable(
          [assignmentsDefault],
          ["table_name", "tenant_id_nullable", "tenant_id_default", "ok"],
        ),
      );
      ...
      console.log("\n## Sprint 8 Default-Hardened Tables\n");
      console.log(
        markdownTable(defaultReports, [
          "table_name",
          "exists",
          "tenant_id_nullable",
          "tenant_id_default",
      ...
      console.log(
        markdownTable(auditLog, ["resource", "total_rows", "nullable_rows"]),
      );
    }
