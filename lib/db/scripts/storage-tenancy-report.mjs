...
import pg from "pg";
import { databaseNodePostgresSslConfig } from "../../../scripts/fieldgrid-database-root-cert.mjs";
...
const productionLike =
  /prod|production|platform\.fieldgrid\.nl/i.test(databaseUrl) ||
  /prod|production/i.test(process.env.APP_ENV ?? "");
if (
  productionLike &&
  process.env.PHASE3_STORAGE_REPORT_ALLOW_PRODUCTION !== "true"
) {
  console.error(
    "Refusing production-like phase 3 storage report without PHASE3_STORAGE_REPORT_ALLOW_PRODUCTION=true.",
  );
  process.exit(1);
}
...
async function main() {
  const client = new Client({
    ...
    ssl: databaseNodePostgresSslConfig(),
  });
  await client.connect();

  try {
    const [documents, assignmentPhotos, reportAttachments, news, policies] =
      await Promise.all([
    ...
    const legacyCount =
      summary.surfaces.reduce((total, surface) => {
        return (
          total +
          Number(
            surface.legacy_storage_paths ?? surface.legacy_hero_paths ?? 0,
          ) +
          Number(surface.unresolved_tenant_id ?? 0) +
          Number(surface.invalid_platform_targets ?? 0)
        );
    ...
    if (asJson) {
      ...
      console.log(
        `- storage policies: ${policies.found.length}/${policies.expected.length} present`,
      );
      if (policies.missing.length)
        console.log(`- missing policies: ${policies.missing.join(", ")}`);
    }

    if (failOnLegacy && legacyCount > 0) {
      console.error(
        `Phase 3 report found ${legacyCount} unresolved legacy/storage/news items.`,
      );
      process.exitCode = 1;
    }
  } finally {
