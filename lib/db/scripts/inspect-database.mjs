...
import { validateEnvironmentIsolation } from "../../../scripts/fieldgrid-environment-isolation-preflight.mjs";
...
const databaseUrl = process.env.FIELDGRID_MIGRATION_DATABASE_URL;
const appEnv = process.env.APP_ENV ?? "unknown";
const reportDir =
  process.env.REPORT_DIR ??
  path.resolve(process.cwd(), "database-inspection", appEnv);

if (!databaseUrl) {
  throw new Error(
    "FIELDGRID_MIGRATION_DATABASE_URL is required for database inspection.",
  );
}
validateEnvironmentIsolation(process.env, { requireMigrationDatabase: true });
...
function renderMarkdown(report) {
  const datasets = Object.fromEntries(
    report.datasets.map((dataset) => [dataset.name, dataset]),
  );
  ...
  const managedRelations = relations.filter((relation) =>
    managedSchemas.has(relation.schema_name),
  );
  const policies = datasets.policies?.rows ?? [];
  const rlsRelations = relations.filter(
  ...
  markdown += section(
    ...
    table(
      schemas.map((schema) => ({
        ...
        category: managedSchemas.has(schema.schema_name)
          ? "supabase-managed"
          : "application/custom",
      })),
...
try {
  ...
  await writeFile(
    path.join(reportDir, "schema-report.md"),
