-- Correct the initial transition allowlist and enforce a non-regressing record
-- lifecycle. Historical ciphertext and its AAD fields remain immutable.
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    IF (NEW.tenant_id, NEW.object_id, NEW.category, NEW.title,
        ...
        NEW.change_reason, NEW.supersedes_record_id, NEW.created_by, NEW.created_at)
      IS DISTINCT FROM
       (OLD.tenant_id, OLD.object_id, OLD.category, OLD.title,
        ...
        OLD.change_reason, OLD.supersedes_record_id, OLD.created_by, OLD.created_at) THEN
...
-- Legacy plaintext may only move toward NULL. This enables an auditable
-- encrypt-then-clear backfill without ever permitting new plaintext writes.
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    END IF;
  ELSIF (NEW.access_info IS DISTINCT FROM OLD.access_info AND NEW.access_info IS NOT NULL)
     OR (NEW.key_info IS DISTINCT FROM OLD.key_info AND NEW.key_info IS NOT NULL)
     OR (NEW.alarm_info IS DISTINCT FROM OLD.alarm_info AND NEW.alarm_info IS NOT NULL) THEN
    RAISE EXCEPTION 'legacy object secret columns may only be cleared after encrypted backfill';
  END IF;
  RETURN NEW;
END;
$$;
...
REVOKE ALL ON FUNCTION public.fieldgrid_reject_legacy_object_secret_write()
  FROM PUBLIC, anon, authenticated, service_role;
