...
function buildPlainHtml(title: string, body: string): string {
  ...
  return [
    ...
    '<div style="background:#08224a;padding:22px 26px;color:#fff;font-weight:700;letter-spacing:.18em">FIELDGRID</div>',
    ...
    if ((setting?.inAppEnabled ?? true) && personnel.length > 0) {
      await db.insert(personnelNotificationsTable).values(
        personnel.map((person) => ({
          ...
          priority,
          sourceLabel: input.fallback.sourceLabel ?? "Fieldgrid",
          href: personnelHref,
        })),
      );
