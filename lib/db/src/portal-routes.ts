...
function mapLegacyCustomerHref(path: string): string {
  ...
  if (customerTicket?.[1])
    return customerPortalRoutes.ticket(decodeSegment(customerTicket[1]));

  const assignment = path.match(/^\/assignments\/([^/?#]+)(.*)?$/u);
  if (assignment?.[1])
    return customerPortalRoutes.assignment(decodeSegment(assignment[1]));

  if (path === "/tickets/customer" || path === "/tickets")
    return customerPortalRoutes.notifications;
  if (path === "/assignments") return "/opdrachten";
  if (path === "/rapportages" || path.startsWith("/rapportages/"))
    return "/rapporten";
  return path;
}
...
function mapLegacyPersonnelHref(path: string): string {
  ...
  if (personnelTicket?.[1])
    return personnelPortalRoutes.ticket(decodeSegment(personnelTicket[1]));

  const assignment = path.match(/^\/assignments\/([^/?#]+)(.*)?$/u);
  if (assignment?.[1])
    return personnelPortalRoutes.assignment(decodeSegment(assignment[1]));

  if (path === "/tickets/personnel" || path === "/tickets")
    return personnelPortalRoutes.notifications;
  if (path === "/assignments") return "/opdrachten";
  return path;
}
...
export function sanitizeCustomerPortalHref(
  rawHref: string | null | undefined,
): string {
...
export function sanitizePersonnelPortalHref(
  rawHref: string | null | undefined,
): string {
  return sanitizePortalHref(rawHref, {
    basePath: "/personeel",
...
export function sanitizeBackofficeHref(
  rawHref: string | null | undefined,
): string {
  const localPath = toLocalPath(rawHref);
  if (!localPath) return "/meldingen";
  return isAllowed(localPath, BACKOFFICE_ALLOWED_PREFIXES)
    ? localPath
    : "/meldingen";
}
