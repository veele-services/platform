...
export function todayDateKey(): string { return dateKey(new Date()); }
...
function addMonths(value: string, months: number, preferredDay: number): string {
...
export function buildAvailabilityRepeatDates(startDate: string, repeatType: AvailabilityRepeat, maxDate: string): string[] {
  const dates: string[] = [];
  ...
  while (current <= maxDate && dates.length < 380) {
    ...
    if (repeatType === "monthly") current = addMonths(startDate, ++step, preferredDay);
  }
  return dates;
}
function normalizeTimeRange(startTime: string, endTime: string): { startTime: string; endTime: string } | string {
  if (!TIME_RE.test(startTime) || !TIME_RE.test(endTime)) return "Ongeldig tijdformaat (verwacht HH:MM)";
...
export async function resolveActivePersonnelForUser(input: { tenantId: string; userId: string }) {
  const [person] = await db.select({ id: personnelTable.id, tenantId: personnelTable.tenantId, isActive: personnelTable.isActive })
    .from(personnelTable)
    .where(and(eq(personnelTable.tenantId, input.tenantId), eq(personnelTable.userId, input.userId), eq(personnelTable.isActive, true)))
    .limit(1);
  return person ?? null;
}

export async function saveWeeklyAvailability(input: { tenantId: string; userId: string; windows: WeeklyAvailabilityInput[]; expectedVersion?: string | null }): Promise<AvailabilityServiceResult<{ version: string }>> {
  if (!input.tenantId) return { ok: false, code: "invalid", message: "Tenantcontext ontbreekt" };
  const seen = new Set<number>();
  for (const window of input.windows) {
    if (window.dayOfWeek < 0 || window.dayOfWeek > 6 || seen.has(window.dayOfWeek)) return { ok: false, code: "invalid", message: "Ongeldige of dubbele weekdag" };
    seen.add(window.dayOfWeek);
    const normalized = normalizeTimeRange(window.startTime, window.endTime);
    if (typeof normalized === "string") return { ok: false, code: "invalid", message: normalized };
  }
  return db.transaction(async (tx) => {
    const [person] = await tx.select({ id: personnelTable.id }).from(personnelTable)
      .where(and(eq(personnelTable.tenantId, input.tenantId), eq(personnelTable.userId, input.userId), eq(personnelTable.isActive, true))).for("update").limit(1);
    if (!person) return { ok: false as const, code: "not_found" as const, message: "Actief personeelsprofiel niet gevonden" };
    const existing = await tx.select().from(availabilityWindowsTable).where(eq(availabilityWindowsTable.personnelId, person.id)).for("update");
    const version = existing.map((row) => row.updatedAt ?? row.createdAt).sort((a, b) => b.getTime() - a.getTime())[0] ?? null;
    if (input.expectedVersion && !sameInstant(input.expectedVersion, version)) return { ok: false as const, code: "conflict" as const, message: "Beschikbaarheid is gewijzigd. Vernieuw en probeer opnieuw." };
    const nextDays = new Set(input.windows.map((w) => w.dayOfWeek));
    const now = new Date();
    for (const stale of existing.filter((row) => !nextDays.has(row.dayOfWeek))) await tx.delete(availabilityWindowsTable).where(eq(availabilityWindowsTable.id, stale.id));
    for (const window of input.windows) await tx.insert(availabilityWindowsTable).values({ personnelId: person.id, ...window, updatedAt: now }).onConflictDoUpdate({ target: [availabilityWindowsTable.personnelId, availabilityWindowsTable.dayOfWeek], set: { startTime: window.startTime, endTime: window.endTime, updatedAt: now } });
    await tx.insert(auditLogTable).values({ tenantId: input.tenantId, userId: input.userId, action: "availability.weekly.save", resource: "availability_windows", resourceId: person.id, metadata: { days: input.windows.length } });
...
export async function saveDateAvailabilityExceptions(input: { tenantId: string; userId: string; exception: DateExceptionInput; maxDate: string }): Promise<AvailabilityServiceResult<{ savedDates: string[]; version: string }>> {
  ...
  const normalized = normalizeTimeRange(input.exception.startTime, input.exception.endTime);
  if (typeof normalized === "string") return { ok: false, code: "invalid", message: normalized };
  if (input.exception.date < todayDateKey()) return { ok: false, code: "invalid", message: "Beschikbaarheid kan niet in het verleden worden aangepast" };
  if (input.exception.date > input.maxDate) return { ok: false, code: "invalid", message: "Datum valt buiten toegestane periode" };
  const dates = buildAvailabilityRepeatDates(input.exception.date, input.exception.repeatType, input.maxDate);
  return db.transaction(async (tx) => {
    const [person] = await tx.select({ id: personnelTable.id }).from(personnelTable)
      .where(and(eq(personnelTable.tenantId, input.tenantId), eq(personnelTable.userId, input.userId), eq(personnelTable.isActive, true))).for("update").limit(1);
    if (!person) return { ok: false as const, code: "not_found" as const, message: "Actief personeelsprofiel niet gevonden" };
    const existing = await tx.select().from(availabilityDayEntriesTable).where(and(eq(availabilityDayEntriesTable.personnelId, person.id), inArray(availabilityDayEntriesTable.date, dates))).for("update");
    const selected = existing.find((row) => row.date === input.exception.date);
    if (input.exception.expectedUpdatedAt && !sameInstant(input.exception.expectedUpdatedAt, selected?.updatedAt ?? null)) return { ok: false as const, code: "conflict" as const, message: "Beschikbaarheid is gewijzigd. Vernieuw en probeer opnieuw." };
    const now = new Date();
