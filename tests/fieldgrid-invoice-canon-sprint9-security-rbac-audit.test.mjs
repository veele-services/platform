...
const invoiceSettingsActions = read("artifacts/backoffice/src/app/actions/invoice-settings.ts");
...
test("Sprint 9 invoice settings actions derive tenant scope server-side only", () => {
  for (const functionName of [
    ...
    assert.match(body, /requireCurrentTenantId\(\)/u, `${functionName} should resolve tenant from auth/session`);
...
test("Sprint 9 invoice write actions require finance permission and keep writes tenant-scoped", () => {
  for (const functionName of ["createInvoice", "finalizeInvoiceDraft", "markInvoiceSent", "markInvoicePaid", "cancelInvoice", "emailInvoice"]) {
    ...
    assert.match(body, /requirePermission\("invoices", "write"\)/u, `${functionName} should require invoice write permission`);
    assert.doesNotMatch(body, /data\.tenantId|input\.tenantId/u, `${functionName} must not trust tenantId from client input`);
  }
  ...
  assert.match(finalization, /UPDATE public\.invoices[\s\S]+WHERE id = \$6 AND tenant_id = \$7/u);
});
...
test("Sprint 9 public payment redirect cannot cross tenant/payment rows", () => {
  ...
  assert.match(body, /innerJoin\(invoicesTable, eq\(paymentsTable\.invoiceId, invoicesTable\.id\)\)/u);
