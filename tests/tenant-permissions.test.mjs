...
const backofficePermissions = readFileSync(
...
const provider = readFileSync(
  "artifacts/backoffice/src/providers/permissions-provider.tsx",
  "utf8",
);
...
for (const [name, source] of [
  ...
  test(`${name} resolves permissions by user and tenant`, () => {
    assert.match(
      source,
      /getUserPermissions\(\s*userId: string,\s*tenantId: string,\s*\)/,
    );
    ...
    assert.match(
      source,
      /innerJoin\(\s*tenantRolesTable,\s*eq\(tenantRolePermissionsTable\.tenantRoleId, tenantRolesTable\.id\),\s*\)/,
    );
    assert.match(source, /eq\(tenantRolesTable\.tenantId, tenantId\)/);
  });
}
