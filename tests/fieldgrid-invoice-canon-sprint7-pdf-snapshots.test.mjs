...
const customerPayRoutePath = "artifacts/klant-pwa/src/app/api/factuur/[id]/pay/route.ts";
...
test("backoffice invoice detail uses invoice snapshots before live tenant settings", () => {
  ...
  assert.match(getInvoice, /companySnapshotJson:\s+invoicesTable\.companySnapshotJson/u);
  assert.match(getInvoice, /paymentSettingsSnapshotJson:\s+invoicesTable\.paymentSettingsSnapshotJson/u);
  assert.match(getInvoice, /templateSnapshotJson:\s+invoicesTable\.templateSnapshotJson/u);
  assert.match(getInvoice, /row\.companySnapshotJson\s+\?\s+normalizeInvoicePdfCompany\(row\.companySnapshotJson\)/u);
  assert.match(getInvoice, /row\.templateSnapshotJson\s+\?\s+normalizeInvoicePdfTemplate\(row\.templateSnapshotJson\)/u);
  assert.match(getInvoice, /row\.paymentSettingsSnapshotJson\s+\?\s+normalizeInvoicePdfPaymentSettings\(row\.paymentSettingsSnapshotJson\)/u);
...
test("PDF renderers are tenant branded, snapshot driven and do not render arbitrary HTML", () => {
  for (const source of [backofficePdf, customerPdf]) {
    ...
    assert.doesNotMatch(source, /dangerouslySetInnerHTML|innerHTML|DOMParser|sanitize-html|<script/iu);
  }
});
...
test("customer PDF route uses immutable snapshots, audited download and customer scoped payment QR", () => {
  ...
  assert.ok(
    body.indexOf("if (!invoice) return new NextResponse") < body.indexOf("const [snapshotRows"),
    "snapshot/live data should load only after scoped invoice lookup",
  );
  assert.ok(
    body.indexOf("const pdfBuffer = await generateCustomerInvoicePdf({") < body.indexOf("await db.insert(auditLogTable).values"),
...
test("PDF download and payment redirect routes return binary/redirect responses", () => {
  assert.match(backofficePdfRoute, /new NextResponse\(new Uint8Array\(pdfBuffer\)/u);
  assert.match(customerPdfRoute, /new NextResponse\(new Uint8Array\(pdfBuffer\)/u);
  assert.ok(existsSync(new URL(`../${customerPayRoutePath}`, import.meta.url)));
  assert.match(customerPayRoute, /getMyCustomerIdentity\(\)/u);
  assert.match(customerPayRoute, /eq\(invoicesTable\.customerId, identity\.customerId\)/u);
  assert.match(customerPayRoute, /eq\(invoicesTable\.tenantId, identity\.tenantId\)/u);
  assert.match(customerPayRoute, /NextResponse\.redirect\(payment\.checkoutUrl, 302\)/u);
});
