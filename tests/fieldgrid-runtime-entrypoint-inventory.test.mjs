...
const script = path.resolve(
  "scripts/fieldgrid-runtime-entrypoint-inventory.mjs",
);
const manifest = JSON.parse(
  fs.readFileSync("docs/runtime-entrypoints/manifest.json", "utf8"),
);
...
function fixtureInventory() {
  if (cachedFixtureInventory) return cachedFixtureInventory;
  fixtureRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "fieldgrid-entrypoints-"),
  );
  const root = fixtureRoot;
  writeFixture(
    root,
    "app/src/actions/accounts.ts",
    `'use server';
  ...
  writeFixture(
    root,
    "app/src/app/api/mollie/webhook/route.ts",
    `
    export async function POST(request) {
  ...
  writeFixture(
    root,
    "app/src/workers/billing-worker.ts",
    `
  ...
  writeFixture(
    root,
    "app/src/cron/nightly-scheduler.ts",
    `
  ...
  writeFixture(
    root,
    "app/tests/actions.test.ts",
    `'use server'; export async function ignoredTestAction() {}`,
  );
  writeFixture(root, "app/docs/route.ts", `export async function GET() {}`);
  writeFixture(
    root,
    "app/scripts/tooling-worker.ts",
    `export function workerResetAuthScheduleNameOnly() {}`,
  );
  writeFixture(
    root,
    "app/src/lib/names-only.ts",
    `export function authWorkerScheduleResetHelper() { return true; }`,
  );
  const output = execFileSync(
    "node",
    [script, "--root", root, "--runtime-roots", "app/src", "--full-json"],
    { encoding: "utf8" },
  );
  cachedFixtureInventory = JSON.parse(output);
  return cachedFixtureInventory;
}
...
test("runtime entrypoint manifest stays compact and separates runtime concepts", () => {
  ...
  assert.equal(
    ...
    "fieldgrid-runtime-entrypoint-inventory-full",
  );
  ...
  assert.ok(
    manifest.counts.uniqueRuntimeNodes <= manifest.counts.classifications,
  );
  ...
  for (const root of [
    ...
    assert.ok(
      manifest.runtimeRoots.includes(root),
      `missing runtime root ${root}`,
    );
  }
});
...
test("scanner fixtures prove representative detections and exclusions", () => {
  ...
  const pairs = inventory.entries.map(
    (entry) => `${entry.kind}:${entry.file}:${entry.name}`,
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("server-action:app/src/actions/accounts.ts:accounts.ts"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("route-handler:app/src/app/api/invoices/route.ts:POST"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("database-callsite:app/src/lib/data.ts:customers"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("rpc-callsite:app/src/lib/data.ts:refresh_customer_rollup"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes(
        "storage-signed-url-issuance:app/src/lib/data.ts:createSignedUrl",
      ),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes(
        "webhook-handler:app/src/app/api/mollie/webhook/route.ts:POST",
      ),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes(
        "worker-entrypoint:app/src/workers/billing-worker.ts:billing-worker.ts",
      ),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes(
        "scheduled-entrypoint:app/src/cron/nightly-scheduler.ts:nightly-scheduler.ts",
      ),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("raw-sql-callsite:app/src/lib/data.ts:database.execute"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:create"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:send"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:fetch"),
    ),
  );
  assert.ok(
    pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:computeRoutes"),
    ),
  );
  assert.ok(
    !pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:info"),
    ),
  );
  assert.ok(
    !pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:increment"),
    ),
  );
  assert.ok(
    !pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:includes"),
    ),
  );
  assert.ok(
    !pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:set"),
    ),
  );
  assert.ok(
    !pairs.some((pair) =>
      pair.includes("provider-boundary:app/src/lib/data.ts:formatMollieStatus"),
    ),
  );
...
test("scanner fixtures prove kind-specific risk severity", () => {
  ...
  assert.ok(
    inventory.counts.uniqueRuntimeNodes <= inventory.counts.classifications,
  );
  assert.equal(
    inventory.counts.providerBoundaries,
    inventory.entries.filter((entry) => entry.kind === "provider-boundary")
      .length,
  );
  const byKind = new Map(
    inventory.entries.map((entry) => [`${entry.kind}:${entry.name}`, entry]),
  );
  assert.equal(byKind.get("webhook-handler:POST").risk.severity, "medium");
  assert.equal(
    byKind.get("storage-signed-url-issuance:createSignedUrl").risk.severity,
    "medium",
  );
  assert.notEqual(
    byKind.get("raw-sql-callsite:database.execute").risk.severity,
    "high",
  );
  assert.equal(
    byKind.get("provider-boundary:create")?.risk.severity,
    "informational",
  );
});
...
test("runtime entrypoint stale-manifest check passes for committed files", () => {
  if (process.platform === "win32") {
    execFileSync(
      ...
      ["/d", "/s", "/c", "pnpm fieldgrid:runtime-entrypoints:check"],
      { stdio: "pipe" },
    );
    return;
  }

  execFileSync("pnpm", ["fieldgrid:runtime-entrypoints:check"], {
    stdio: "pipe",
  });
});
