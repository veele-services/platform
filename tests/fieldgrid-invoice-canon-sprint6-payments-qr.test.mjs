...
test("Sprint 6 gates Mollie payment creation behind tenant invoice payment settings", () => {
  ...
  assert.match(singlePayment, /prepareDirectPaymentIntent/u);
  assert.match(singlePayment, /createProviderPayment/u);
  assert.ok(
    singlePayment.indexOf("requireMolliePaymentsEnabled(tenantId)") <
      singlePayment.indexOf("prepareDirectPaymentIntent"),
    "tenant settings should be checked before creating the durable provider intent",
  );

  const collectivePayment = functionBlock(
    invoicesAction,
    "createCollectiveInvoicePayment",
  );
  assert.match(collectivePayment, /requireMolliePaymentsEnabled\(tenantId\)/u);
  assert.match(collectivePayment, /prepareCollectionPaymentIntent/u);
  assert.match(collectivePayment, /createProviderPayment/u);
  assert.ok(
    collectivePayment.indexOf("requireMolliePaymentsEnabled(tenantId)") <
      collectivePayment.indexOf("prepareCollectionPaymentIntent"),
...
test("Sprint 6 renders configured payment link and server-side QR in backoffice PDFs", () => {
  ...
  assert.match(pdfRoute, /\/backoffice-api\/invoices\/\$\{invoice\.id\}\/pay/u);
