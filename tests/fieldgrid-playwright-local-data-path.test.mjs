...
test("contract: runtime data-path evidence is produced by gateway to real PostgREST requests", () => {
  ...
  assert.match(
    source,
    /gatewayJson\(\s*`\/rest\/v1\/customer_assignment_projection\?id=eq\.\$\{tenantAAssignment\}/,
  );
  assert.match(
    source,
    /gatewayJson\(\s*`\/rest\/v1\/customer_assignment_projection\?id=eq\.\$\{tenantBAssignment\}/,
  );
  assert.doesNotMatch(
    source,
    /gatewayJson\(`\/rest\/v1\/assignments\?id=eq\.\$\{tenant[AB]Assignment\}/,
  );
...
test("contract: gateway preserves method, body, query, headers and PostgREST response details", () => {
  ...
  assert.match(source, /body,\s*duplex: ["']half["']/);
  for (const header of [
    "authorization",
    "apikey",
    "accept",
