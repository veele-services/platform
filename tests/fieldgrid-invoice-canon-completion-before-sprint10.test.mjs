...
const migration = read("lib/db/migrations/20260710200000_invoice_canon_credit_collection_payments.sql");

test("canon document numbering supports invoices, credit notes and collections", () => {
  assert.match(invoicesSchema, /INVOICE_NUMBER_DOCUMENT_TYPES = \["invoice", "credit_note", "invoice_collection"\]/u);
  assert.match(invoicesSchema, /documentType: varchar\("document_type"/u);
  assert.match(invoicesSchema, /uniqueIndex\("invoice_number_sequences_tenant_settings_period_idx"\)[\s\S]+table\.documentType[\s\S]+table\.periodKey/u);
  ...
  assert.match(finalization, /documentType: invoice\.type === "credit_note" \? "credit_note" : "invoice"/u);
});
...
test("payments support manual and provider flows through canonical allocations", () => {
  assert.match(paymentsSchema, /PAYMENT_SOURCE_TYPES = \["invoice", "invoice_collection"\]/u);
  assert.match(paymentsSchema, /PAYMENT_METHODS = \["mollie", "manual_bank", "cash", "correction", "settlement", "other"\]/u);
  assert.match(paymentsSchema, /paymentAllocationsTable = pgTable\(\s*"payment_allocations"/u);
  assert.match(paymentsSchema, /invoiceId: uuid\("invoice_id"\)\.references/u);

  const manualBody = functionBlock(paymentActions, "registerManualInvoicePayment");
  ...
  assert.match(webhook, /paymentAllocationsTable/u);
  assert.match(webhook, /sourceType: paymentsTable\.sourceType/u);
  assert.match(webhook, /payment\.sourceType === "invoice_collection"/u);
  assert.match(webhook, /collectionStatus: "collection_paid"/u);
});

test("collection invoices carry VZF numbers, tenant metadata and line snapshots", () => {
  assert.match(batchesSchema, /collectionNumber: varchar\("collection_number"/u);
  ...
  const backofficeBody = functionBlock(invoiceActions, "createCollectiveInvoicePayment");
  assert.match(backofficeBody, /claimOfficialInvoiceCollectionNumberInTransaction/u);
  assert.match(backofficeBody, /sourceType: "invoice_collection"/u);
  assert.match(backofficeBody, /tenantId,\s+batchId: batch\.id/u);
  assert.match(backofficeBody, /invoiceNumberSnapshot: invoice\.invoiceNumber/u);

  assert.match(customerPayments, /tenantId:\s+auth\.tenantId/u);
  assert.match(customerPayments, /sourceType:\s+"invoice_collection"/u);
  assert.match(customerPayments, /createdByActorType:\s+"customer_user"/u);
  assert.match(customerPayments, /invoiceNumberSnapshot: invoice\.invoiceNumber/u);
});
...
test("migration is ordered and backfills without renumbering existing invoices", () => {
  ...
  assert.match(migration, /CREATE TABLE IF NOT EXISTS public\.payment_allocations/u);
  assert.match(migration, /UPDATE public\.payments payment[\s\S]+source_id = COALESCE\(payment\.source_id, payment\.invoice_id\)/u);
  assert.match(migration, /invoice_number_snapshot/u);
  assert.doesNotMatch(migration, /UPDATE public\.invoices\s+SET invoice_number/u);
});
