...
test("FG-HARD-025: no surface treats a reset or activation code as a password", () => {
  ...
  assert.doesNotMatch(invite, /temporaryPassword|buildTemporaryPasswordEmail|password:\s*(?:code|challenge\.code|opts\.)/u);
...
test("challenge lifecycle stores hashes only and is atomic under concurrency", () => {
  const migration = read("lib/db/migrations/20260718180000_complete_credential_recovery.sql");
  ...
  assert.doesNotMatch(migration, /(?:raw_)?(?:code|token|password)\s+(?:text|varchar)/u);
...
test("redirects, cookies, service role and test email transport fail closed", () => {
  ...
  assert.match(surfaceActions.map(read).join("\n"), /FIELDGRID_RECOVERY_ALLOWED_ORIGINS/u);
  ...
  for (const path of surfaceActions) {
    ...
    assert.ok(
      /secure:\s*(?:process\.env\.NODE_ENV === "production"|recoveryOrigin\(\)\.startsWith\("https:\/\/"\))/u.test(source),
  ...
  assert.ok(
    email.indexOf("FIELDGRID_EMAIL_TEST_OUTBOX_PATH") < email.indexOf("provider = await resolveActiveProvider"),
  );
});
...
test("recovery UI and routes use safe server-side states", () => {
  ...
  const request = read("artifacts/klant-pwa/src/app/api/auth/password-reset/request/route.ts");
  const verify = read("artifacts/klant-pwa/src/app/api/auth/password-reset/verify/route.ts");
  const complete = read("artifacts/klant-pwa/src/app/api/auth/password-reset/complete/route.ts");
