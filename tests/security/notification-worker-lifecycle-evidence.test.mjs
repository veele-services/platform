...
const worker = readFileSync("artifacts/api-server/src/lib/notification-worker.ts", "utf8");
const migration = readFileSync(
  "lib/db/migrations/20260820111234_notification_worker_lifecycle_evidence.sql",
  "utf8",
);
const runtime = readFileSync("scripts/fieldgrid-notification-worker-runtime.mts", "utf8");
...
test("claim, attempt evidence and terminal finalization are transactionally coupled", () => {
  assert.match(worker, /notification_delivery_attempts[\s\S]*status = 'processing'/u);
  ...
  assert.match(migration, /notification_delivery_queue_terminal_evidence_check/u);
...
test("uncertain provider effects never become an automatic blind redelivery", () => {
  ...
  assert.match(worker, /status IN \('failed', 'outcome_pending', 'partial'\)/u);
...
test("push outcomes retain safe endpoint-level evidence", () => {
  ...
  assert.match(worker, /status: errors\.length > 0 \? "partial" : "sent"/u);
  assert.doesNotMatch(worker, /targetOutcomes\.push\(\{[\s\S]{0,180}endpoint:/u);
