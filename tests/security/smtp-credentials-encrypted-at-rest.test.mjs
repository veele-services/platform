...
test("tenant SMTP schema is additive and rejects new plaintext writes", async () => {
  ...
  assert.match(migration, /ADD COLUMN IF NOT EXISTS smtp_password_encrypted text/u);
  assert.match(migration, /fieldgrid_reject_plaintext_smtp_password/u);
  assert.match(migration, /SECURITY INVOKER/u);
  assert.match(migration, /REVOKE ALL[\s\S]*PUBLIC, anon, authenticated, service_role/u);
  assert.doesNotMatch(migration, /FIELDGRID_EMAIL_CONFIG_ENCRYPTION_KEY|base64:|hex:/u);
});
...
test("normal tenant SMTP reads and writes use only the encrypted credential", async () => {
  ...
  const settings = await read("artifacts/backoffice/src/app/actions/settings.ts");

  assert.match(service, /smtpPasswordEncrypted: organizationSettingsTable\.smtpPasswordEncrypted/u);
  ...
  assert.doesNotMatch(settings, /updateData\.smtpPassword = data\.smtpPassword/u);
  assert.match(settings, /smtpPasswordConfigured: Boolean\(r\.smtpPasswordEncrypted\)/u);
});
