...
test("every validation group checks out and proves the recorded GitHub SHA", () => {
  const checkoutGroups = workflow.match(/uses: actions\/checkout@v4/gu) ?? [];
  const explicitRefs = workflow.match(/ref: \$\{\{ github\.sha \}\}/gu) ?? [];
...
test("exact-head validation includes every authoritative gate", () => {
  for (const command of [
    ...
    assert.ok(workflow.includes(command), `missing authoritative command: ${command}`);
  }
});
...
test("aggregate is fail-closed and workflow cannot deploy", () => {
  ...
  assert.doesNotMatch(workflow, /fieldgrid-atomic-release-activate|fieldgrid-deploy-health-gate\.sh\s+(?:staging|production)|db:migrate/u);
});
