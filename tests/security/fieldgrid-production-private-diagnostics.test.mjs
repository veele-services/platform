...
test('diagnostics bind private regular files to the exact numeric run and distinguish retained reruns', t => {
  ...
  assert.equal(result.rehearsals.length, 1);
  assert.equal(result.rehearsals[0].categories.missingRelation, true);
  assert.deepEqual(result.rehearsals[0].sqlStates, ['42P01']);
  ...
  const rerunDirectory = path.join(root, 'b'.repeat(12) + '-12345-Ab123x'); mkdirSync(rerunDirectory);
  writeFileSync(path.join(rerunDirectory, 'private-operations.log'), "code: '42501'", { mode: 0o600 });
  const reruns = diagnosePrivateRun('12345', options).rehearsals;
  assert.equal(reruns.length, 2);
  assert.notEqual(reruns[0].backupId, reruns[1].backupId);
  assert.deepEqual(reruns[1].sqlStates, ['42501']);
  assert.throws(() => diagnosePrivateRun('12345.*', options));
  for (let index = 0; index < 19; index++) mkdirSync(path.join(root, 'c'.repeat(12) + '-12345-' + String(index).padStart(6, '0')));
  assert.throws(() => diagnosePrivateRun('12345', options));
});
