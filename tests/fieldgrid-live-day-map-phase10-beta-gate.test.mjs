...
test("phase 10 keeps live day map beta-gated and preserves planning views", () => {
  ...
  assert.match(registry, /id: "tenant-planning"/);
  assert.doesNotMatch(registry, /href: "\/planning\?view=map"/);
...
test("phase 10 map data server action is tenant and permission scoped", () => {
  ...
  assert.match(
    planning,
    /createEmptyPlanningDayMapData\(date, \{ accessDenied: true \}\)/,
  );
  ...
  assert.match(
    planning,
    /eq\(assignmentRouteContextsTable\.tenantId,\s*tenantId\)/,
  );
});
...
test("phase 10 route provider has deterministic fallback without external API keys", () => {
  ...
  assert.doesNotMatch(
    routesClient,
    /TRAFFIC_AWARE_OPTIMAL|computeRouteMatrix|optimizeWaypointOrder/,
  );
