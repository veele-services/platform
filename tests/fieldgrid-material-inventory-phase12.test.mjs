...
test("phase 12 readiness plan is non-destructive and depends on phase 11", () => {
  ...
  assert.ok(
    plan.dependsOn.includes("fieldgrid-material-inventory-phase-11-hardening"),
  );
});
...
test("phase 12 production gates include migration, security, PWA, storage and rollback", () => {
  ...
  for (const gate of PHASE12_PRODUCTION_GATES) {
    assert.ok(
      plan.productionGates.includes(gate),
      `missing production gate ${gate}`,
    );
  }

  assert.ok(
    plan.productionGates.includes("empty_database_migration_smoke_green"),
  );
  assert.ok(
    plan.productionGates.includes("staging_copy_migration_smoke_green"),
  );
...
test("phase 12 workflow runs readiness, tests, typecheck and build", () => {
  ...
  assert.ok(workflow.includes("pnpm -r --if-present run build"));
});
