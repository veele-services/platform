...
const migration = read(
  "lib/db/migrations/20260718120000_durable_staffing_lifecycle.sql",
);
...
async function ensureCurrentDayAvailability(client, personnelIds) {
  await client.query(
    `insert into public.availability_day_entries
      (personnel_id, date, start_time, end_time)
     select personnel_id, to_char(current_date, 'YYYY-MM-DD'), '00:00', '23:59'
       from unnest($1::uuid[]) as personnel_id
     on conflict (personnel_id, date) do update
...
test("Phase 2A product paths use canonical transitions instead of hard-deleting lifecycle rows", () => {
  ...
  assert.doesNotMatch(
    assignmentActions,
    /\.delete\(assignmentPersonnelTable\)/u,
  );
  assert.doesNotMatch(planningActions, /\.delete\(assignmentPersonnelTable\)/u);
  assert.doesNotMatch(
    interestStaffing,
    /delete\s+from\s+public\.assignment_personnel/iu,
  );
});
...
test("Phase 2A migration preserves snapshots, remains previous-release compatible, and restricts public RPC access", () => {
  assert.match(
    migration,
    /assignment_personnel_unique_idx[\s\S]*assignment_id, personnel_id/iu,
  );
  ...
  assert.match(
    migration,
    /revoke all on function public\.transition_assignment_staffing[\s\S]*from public, anon, authenticated/iu,
  );
  assert.match(
    migration,
    /grant execute on function public\.transition_assignment_staffing[\s\S]*to service_role/iu,
  );
  assert.match(
...
test("Phase 2A surfaces use participant progression, positioned actual overlays, and a real multi-context journey", () => {
  assert.match(
    participantProgress,
    /assignment\.participantStatus \?\? assignment\.status/u,
  );
  assert.match(
    personnelAssignmentActions,
    /assigned:\s*\["seen", "en_route", "in_progress"\]/u,
  );
  assert.match(
    planboard,
  ...
  assert.match(
    browserJourney,
    /goEnRouteAndStart\(participantOne, participantOnePersonnelId\)/u,
  );
  assert.match(
    browserJourney,
    /goEnRouteAndStart\(participantTwo, participantTwoPersonnelId\)/u,
  );
  assert.match(
    e2eAuth,
    /20000000-0000-4000-8000-000000000107[\s\S]*phase2-personnel@tenant-a\.runtime\.fieldgrid\.test/u,
  );
...
test("Phase 2A planboard free-state badge keeps WCAG AA text contrast", () => {
  ...
  const scheduledAssignmentsStart = planboard.indexOf(
    "person.scheduledAssignments.map",
    freeStateStart,
  );
  assert.notEqual(freeStateStart, -1);
  assert.ok(scheduledAssignmentsStart > freeStateStart);
  const freeState = planboard.slice(freeStateStart, scheduledAssignmentsStart);
...
test(
  ...
  async () =>
    withRollback(async (client) => {
      ...
      await ensureCurrentDayAvailability(client, [FIXTURE.personnelA]);
      await client.query(
      ...
      await expectDbError(
        client,
        "wrong_tenant",
        () => staffing(client, FIXTURE.personnelB, "assign"),
        "42501",
      );
      ...
      const duplicateAssign = await staffing(
        client,
        FIXTURE.personnelA,
        "assign",
      );
      ...
      assert.equal(
        duplicateAssign.rows[0].assignment_personnel_id,
        firstLinkId,
      );
      ...
      await expectDbError(
        client,
        "blank_reason",
        () => staffing(client, FIXTURE.personnelA, "unassign", ""),
        "22023",
      );
      await expectDbError(
        client,
      ...
      const unassigned = await staffing(
        client,
        FIXTURE.personnelA,
        "unassign",
        "Planning gewijzigd",
      ...
      const duplicateUnassign = await staffing(
        client,
        FIXTURE.personnelA,
        "unassign",
        "Planning gewijzigd",
      ...
      assert.deepEqual(
        executions.rows.map((row) => row.participant_status).sort(),
        ["assigned", "removed"],
      );
    }),
);
...
test(
  ...
  async () =>
    withRollback(async (client) => {
      ...
      await ensureCurrentDayAvailability(client, [
        FIXTURE.personnelA,
        FIXTURE.personnelA2,
      ]);
      await client.query(
      ...
      const fullyStaffed = await staffing(
        client,
        FIXTURE.personnelA2,
        "assign",
      );
      ...
      await expectDbError(
        ...
        "started_unassign",
        () =>
          staffing(client, FIXTURE.personnelA, "unassign", "Te laat gewijzigd"),
        "23514",
      );
      await expectDbError(
        client,
      ...
      assert.deepEqual(
        aggregate.rows[0].actual_started_at,
        aggregate.rows[0].earliest_start,
      );
      assert.deepEqual(
        aggregate.rows[0].actual_completed_at,
        aggregate.rows[0].latest_end,
      );
    }),
);
