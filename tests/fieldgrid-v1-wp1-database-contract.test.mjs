...
test('payment preflight blocks malformed/live rows but leaves Mollie terminality to provider truth',()=>{
  ...
  const seeded={id:'50000000-0000-4000-8000-000000000001',tenant_id:'10000000-0000-4000-8000-000000000001',invoice_id:invoice,payment_method:'mollie',mollie_payment_id:'tr_staging_demo_40000000_paid',checkout_url:`https://www.mollie.com/checkout/staging-demo/${invoice}`,paid_at:'2026-01-01T00:00:00Z',status:'paid'};
  const seededData={...data([seeded]),invoices:[{id:invoice,tenant_id:seeded.tenant_id,notes:'VEELE_STAGING_DEMO_DEN_HAAG: seeded'}],payment_allocations:[{payment_id:seeded.id}]};
  assert.equal(paymentBlockers(seededData),0);
