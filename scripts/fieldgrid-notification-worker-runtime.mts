...
async function enqueue(input: {
  ...
  const result = await client.query(
    `insert into public.notification_delivery_queue (
     ...
     ) values ($1,$2,$3,$4,$5,$6,$7,'runtime@example.test','Runtime notification',
    ...
    [
      ...
      `${prefix}:${id}`,
      JSON.stringify(input.recipientType === "management" || !input.recipientType
...
try {
  await client.query(`delete from public.notification_delivery_queue where title like $1`, [`${prefix}:%`]);
  ...
  const suspendedResult = await runEmail({
    ...
    afterClaim: async () => {
      await client.query(`update public.tenants set is_active=false, status='suspended' where id=$1`, [tenantA]);
    },
  ...
  await client.query(`update public.tenants set is_active=true, status='active' where id=$1`, [tenantA]);
  ...
  assert.equal((await queueState(moduleDisabled.id)).last_error, "module_disabled");
  await client.query(
  ...
  await runEmail({
    ...
    afterClaim: async () => {
      await client.query(`update public.personnel set is_active=false where id=$1`, [FIXTURE.personnel.a]);
    },
  });
  assert.equal((await queueState(recipientDisabled.id)).last_error, "recipient_inactive");
  await client.query(`update public.personnel set is_active=true where id=$1`, [FIXTURE.personnel.a]);
  ...
  await runEmail({
    ...
    afterClaim: async () => {
      await client.query(`update public.personnel set notification_email_enabled=false where id=$1`, [FIXTURE.personnel.a]);
    },
  });
  assert.equal((await queueState(preferenceDisabled.id)).last_error, "notification_disabled");
  await client.query(`update public.personnel set notification_email_enabled=true where id=$1`, [FIXTURE.personnel.a]);
  ...
  const notificationDisabled = await enqueue({ eventKey: event.rows[0].event_key });
  await runEmail({
    workerId: "runtime-notification-disabled-after-claim",
    afterClaim: async () => {
      await client.query(`update public.notification_event_settings set email_enabled=false where event_key=$1`, [event.rows[0].event_key]);
    },
  });
  assert.equal((await queueState(notificationDisabled.id)).last_error, "notification_disabled");
  await client.query(`update public.notification_event_settings set email_enabled=true where event_key=$1`, [event.rows[0].event_key]);
  ...
  await runEmail({ deliveryOverride: async () => { crossTenantDeliveries += 1; return delivered(); } });
  assert.equal(crossTenantDeliveries, 0);
  assert.equal((await queueState(crossTenant.id)).last_error, "recipient_inactive");
  ...
  assert.deepEqual((await attempts(staleSafe.id)).map((row) => row.status), ["abandoned", "sent"]);

  const uncertain = await enqueue({});
  await makeStaleProcessing(uncertain.id, true);
  let uncertainDeliveries = 0;
  const uncertainResult = await runEmail({ deliveryOverride: async () => { uncertainDeliveries += 1; return delivered(); } });
  ...
  assert.equal((await retryFailedNotifications({ limit: 1, logger })).requeued, 1);
  assert.equal((await queueState(uncertain.id)).status, "retry");
  await client.query(`delete from public.notification_delivery_queue where id=$1`, [uncertain.id]);

  const transient = await enqueue({});
  await runEmail({
    workerId: "runtime-transient-provider",
    deliveryOverride: async () => { throw new Error("transient provider failure"); },
  });
  assert.equal((await queueState(transient.id)).status, "retry");
  assert.equal((await attempts(transient.id))[0].status, "retry");
  await client.query(`delete from public.notification_delivery_queue where id=$1`, [transient.id]);

  const maxAttempts = await enqueue({ maxAttempts: 1 });
  await runEmail({
    workerId: "runtime-max-attempts",
    deliveryOverride: async () => { throw new Error("permanent after max attempts"); },
  });
  ...
  await assert.rejects(
    runEmail({
      ...
      deliveryOverride: async () => { attemptFailureDeliveries += 1; return delivered(); },
    }),
  );
  ...
  await client.query(`delete from public.notification_delivery_queue where id=$1`, [attemptFailure.id]);
  await client.query(`drop trigger fieldgrid_runtime_reject_notification_attempt on public.notification_delivery_attempts`);
  await client.query(`drop function public.fieldgrid_runtime_reject_notification_attempt()`);
  ...
  await assert.rejects(
    runEmail({
      ...
      deliveryOverride: async () => { finalizationDeliveries += 1; return delivered(); },
    }),
  );
  assert.equal(finalizationDeliveries, 1);
  assert.equal((await queueState(finalizationFailure.id)).status, "processing");
  assert.equal((await attempts(finalizationFailure.id))[0].status, "processing");
  await client.query(`drop trigger fieldgrid_runtime_reject_notification_finalization on public.notification_delivery_queue`);
  await client.query(`drop function public.fieldgrid_runtime_reject_notification_finalization()`);
  await client.query(
    `update public.notification_delivery_queue
  ...
  assert.equal(finalizationDeliveries, 1);
  assert.equal((await queueState(finalizationFailure.id)).status, "outcome_pending");

  const webQueue = await enqueue({ channel: "push", recipientType: "personnel", personnelId: FIXTURE.personnel.a });
  ...
  await client.query(
    ...
    [webGood, webInvalid, tenantA, FIXTURE.personnel.a, `https://push.runtime/${webGood}`, `https://push.runtime/${webInvalid}`],
  );
  const webResult = await processNotificationQueue({
    channels: ["push"], limit: 10, pushRatePerRun: 10, sendDelayMs: 0, lockSeconds: 1, logger,
    webPushSender: async (subscription) => subscription.endpoint.endsWith(webInvalid)
      ? { success: false, status: 410, error: "gone", permanent: true }
      : { success: true, status: 201 },
    fcmPushSender: async () => ({ success: false, status: 0, error: "unexpected", permanent: false }),
  });
  ...
  assert.equal((await client.query(`select is_active from public.push_subscriptions where id=$1`, [webInvalid])).rows[0].is_active, false);
  await client.query(`delete from public.push_subscriptions where id=$1`, [webGood]);
  await client.query(`update public.push_subscriptions set is_active=true where id=$1`, [webInvalid]);
  const permanentQueue = await enqueue({ channel: "push", recipientType: "personnel", personnelId: FIXTURE.personnel.a });
  const permanentResult = await processNotificationQueue({
    channels: ["push"], limit: 10, pushRatePerRun: 10, sendDelayMs: 0, lockSeconds: 1, logger,
    webPushSender: async () => ({ success: false, status: 410, error: "gone", permanent: true }),
    fcmPushSender: async () => ({ success: false, status: 0, error: "unexpected", permanent: false }),
  });
  assert.equal(permanentResult.failed, 1);
  assert.equal((await queueState(permanentQueue.id)).status, "failed");
  await client.query(`delete from public.push_subscriptions where id=$1`, [webInvalid]);

  const fcmQueue = await enqueue({ channel: "push", recipientType: "personnel", personnelId: FIXTURE.personnel.a });
  ...
  await client.query(
    ...
    [fcmGood, fcmInvalid, tenantA, FIXTURE.personnel.a, `runtime-token-${fcmGood}`, `runtime-token-${fcmInvalid}`],
  );
  const fcmResult = await processNotificationQueue({
    channels: ["push"], limit: 10, pushRatePerRun: 10, sendDelayMs: 0, lockSeconds: 1, logger,
    webPushSender: async () => ({ success: false, status: 0, error: "unexpected", permanent: false }),
    fcmPushSender: async (token) => token.endsWith(fcmInvalid)
      ? { success: false, status: 404, error: "unregistered", permanent: true }
      : { success: true, status: 200, messageId: "runtime-fcm-message" },
  });
  assert.equal(fcmResult.partial, 1);
  assert.equal((await queueState(fcmQueue.id)).response.targetOutcomes.length, 2);
  assert.equal((await client.query(`select is_active from public.native_push_device_tokens where id=$1`, [fcmInvalid])).rows[0].is_active, false);
  await client.query(`delete from public.native_push_device_tokens where id in ($1,$2)`, [fcmGood, fcmInvalid]);
  ...
  try {
    ...
    assert.equal((await rlsClient.query(`select count(*)::int as count from public.notification_delivery_queue where id=$1`, [rlsA.id])).rows[0].count, 1);
    assert.equal((await rlsClient.query(`select count(*)::int as count from public.notification_delivery_queue where id=$1`, [rlsB.id])).rows[0].count, 0);
    assert.equal((await rlsClient.query(`select count(*)::int as count from public.notification_delivery_attempts where id=$1`, [rlsBAttempt])).rows[0].count, 0);
    assert.equal((await rlsClient.query(`update public.notification_delivery_queue set title='forbidden' where id=$1`, [rlsB.id])).rowCount, 0);
  ...
  await client.query(`drop trigger if exists fieldgrid_runtime_reject_notification_attempt on public.notification_delivery_attempts`).catch(() => {});
  await client.query(`drop function if exists public.fieldgrid_runtime_reject_notification_attempt()`).catch(() => {});
  await client.query(`drop trigger if exists fieldgrid_runtime_reject_notification_finalization on public.notification_delivery_queue`).catch(() => {});
  await client.query(`drop function if exists public.fieldgrid_runtime_reject_notification_finalization()`).catch(() => {});
  await client.query(`update public.tenants set is_active=true, status='active' where id=$1`, [tenantA]).catch(() => {});
  await client.query(`update public.personnel set is_active=true where id=$1`, [FIXTURE.personnel.a]).catch(() => {});
  await client.query(`update public.personnel set notification_email_enabled=true where id=$1`, [FIXTURE.personnel.a]).catch(() => {});
  await client.query(`delete from public.notification_delivery_queue where id=any($1::uuid[])`, [createdQueueIds]).catch(() => {});
  await client.end();
  await pool.end();
}
