  ...
  return {
    ...
    checks: [
      ...
      },
      {
        id: "FG-SMOKE-LOGIN",
        boundary: "login",
        testIds: ["FG-PLATFORM-001", "FG-RBAC-001"],
      },
      {
        id: "FG-SMOKE-MODULES",
        boundary: "modules",
        testIds: ["FG-MODULE-001", "FG-MODULE-003", "FG-MODULE-005"],
      },
      {
        id: "FG-SMOKE-SECTORS",
        boundary: "sectoren",
        testIds: ["FG-SECTOR-001", "FG-SECTOR-002", "FG-SECTOR-006"],
      },
      {
        id: "FG-SMOKE-STORAGE",
        boundary: "storage",
        testIds: ["FG-STORAGE-001", "FG-STORAGE-002", "FG-STORAGE-007"],
      },
      {
        id: "FG-SMOKE-PDF-DOWNLOADS",
        boundary: "pdf-downloads",
        testIds: ["FG-DATA-004", "FG-DATA-005", "FG-AUDIT-001"],
      },
      {
      ...
      {
        id: "FG-SMOKE-SUPPORT",
        boundary: "support grants",
        testIds: ["FG-SUPPORT-001", "FG-SUPPORT-002", "FG-SUPPORT-005"],
      },
      {
        id: "FG-SMOKE-AUDIT",
        boundary: "audit",
...
export function validatePhase7StagingSmokePlan(
  plan = buildPhase7StagingSmokePlan(),
) {
  const errors = [];
  ...
  for (const requiredCheck of [
    ...
    if (!checkIds.has(requiredCheck))
      errors.push(`${requiredCheck} ontbreekt in het smokeplan.`);
  }

  if (plan.destructive)
    errors.push("Fase 7 smokeplan mag niet destructief zijn.");
  if (plan.mutatesExistingTenants)
    errors.push("Fase 7 smokeplan mag bestaande tenants niet wijzigen.");
...
console.log(
  `Mutates existing tenants: ${plan.mutatesExistingTenants ? "yes" : "no"}`,
);
...
console.log("");
console.log(
  "Use --json voor machine-readable output of --check voor CI-validatie.",
);
