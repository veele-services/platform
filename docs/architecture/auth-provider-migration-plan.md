...
4. **Session compatibility.** Define how old provider sessions are revoked, where current repository seams fall short of complete all-session refresh-token revocation, and how new host-only Fieldgrid cookies are issued after reauthentication.
...
- Session revocation must clear Fieldgrid cookies and revoke provider refresh sessions as a target capability; do not state complete all-session revocation is already implemented until the provider seam and runtime evidence prove it.
- Move toward server-only/`HttpOnly` cookies where compatible, but first prove refresh/login/logout behavior and never claim `HttpOnly` until implemented and browser-tested.
- While browser-readable cookies remain, document the XSS tradeoff and pair `SameSite=Lax` with explicit CSRF protections for unsafe requests.
