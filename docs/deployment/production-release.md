...
For a failed private rehearsal, supply its numeric `preflight_run_id` to the inventory workflow. Its diagnostic report contains only fixed error categories, allowed SQLSTATE/process codes and migration names from the reviewed source. Reruns retain the same GitHub run ID, so up to twenty retained rehearsals are reported separately by their unique backup identity; unreadable logs remain explicitly unavailable. Raw command logs and database payloads remain private on the VPS. New rehearsal failures include the same bounded diagnostics automatically.
...
The workflow fixes `PLATFORM_HOSTS` to `admin.fieldgrid.nl,platform.fieldgrid.nl,app.fieldgrid.nl,fieldgrid.nl` for both builds and the saved runtime environment. These hosts are classified as platform hosts instead of tenant domains. The shared personnel entry at `app.fieldgrid.nl/personeel` requires an organization code or an authenticated account association; platform classification does not grant tenant access. Customer login requires the customer's own verified tenant host: `app.fieldgrid.nl/klant/healthz` is a service probe, not evidence that the shared host supports customer login. An empty production database still needs its first tenant and accounts before tenant login can work. Staging hosts remain blocked by the existing environment guard, and tenant subdomains still require their existing verified, active tenant binding. The hostname list does not configure Caddy or make an apex path reachable by itself.
