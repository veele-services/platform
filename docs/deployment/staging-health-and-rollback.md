...
When a new root test is intentionally environment-gated in this no-database lane, set `FIELDGRID_BASELINE_DIFF_ALLOWED_ADDITIONAL_SKIPS_JSON` to the exact JSON array of allowed skipped test identifiers reported by the gate. An entry may remain after merge while that identifier is still skipped, so an exact-head run stays reproducible; remove it in the same change that makes the test executable. A dedicated Node test-event reporter records each skipped test as relative test file, reconstructed suite path, leaf name and declaration occurrence while excluding skipped suites. The gate rejects unlisted new skips, count/evidence mismatches, identifiers not skipped by the candidate, duplicate allowlist entries and malformed JSON. Reports contain only the allowlist count and SHA-256 digest, never its raw configuration values.
