...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Tenant core | `tenants`, `tenant_users` | Basis bestaat met `slug`, `name`, `is_active`; tenant membership bestaat. Lifecycle is nog te beperkt. | `tenant_config` + `direct_tenant_id` voor membership | P0 | Ja: `status`, `plan_key`, lifecycle/audit/provisioningvelden. | Host-first active/suspended/archived, membership denial, geen productie-default fallback. | Fase 1 |
...
| Customer users en portal identity | `customer_users`, `customer_portal_preferences` | Tenant/customer/user scope bestaat; portal is nog niet host-first tenant-bound. | `direct_tenant_id` | P0 | Geen schema nu; resolver/runtime later. | Klant krijgt geen tenantkeuze, host Tenant A + Tenant B user ambiguity faalt veilig. | Fase 8 |
...
| Personnel | `personnel`, availability, qualifications, personnel notifications/tickets | `personnel.tenant_id` bestaat; app gebruikt veel `personnel.user_id`. Portal hostbinding ontbreekt nog. | `direct_tenant_id` | P0 | Geen basis-migratie; portal/runtime guards later. | Personeel ziet alleen eigen tenant en eigen/toegewezen opdrachten. | Fase 8 |
...
| Modules | `modules`, `tenant_modules`, dependencies | Ontbreekt. | `needs_migration` + `tenant_config` | P0 | Ja: modules, dependencies, tenant_modules. | Module uit via UI/direct URL/server action/API faalt. | Fase 3 |
...
| Portal routing | backoffice, API, klant-PWA, personeel-PWA | Backoffice is host-first; API/PWA's nog niet uniform host-bound. | `needs_migration` in runtime, geen DB-strategie | P0 | Geen DB-migratie; shared resolver runtime later. | Host Tenant A kan Tenant B-context niet activeren; customer/personnel ambiguity faalt veilig. | Fase 1/8 |
