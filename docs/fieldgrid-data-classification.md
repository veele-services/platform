...
| Support access | `support_access_grants`, `support_access_audit_log` | `partial` | `platform_only` + tenant-scoped support | P0 | Max TTL, break-glass reason en securitydashboard 2.0 zijn geleverd; runtime-proof en Playwright-smoke blijven open. | `FG-SUPPORT-*`, `FG-AUDIT-002`. | 10 |
...
| Audit logging | `audit_log`, support audit | `partial` | `direct_tenant_id` voor tenant-audit + `platform_only` voor platform-audit | P1/P0 | Sprint 10 levert centraal security-auditcontract en dashboardfilters; extra runtimepaden moeten contractueel blijven loggen. | `FG-DATA-009`, `FG-AUDIT-*`. | 8/10 |
...
| Modules | `modules`, `tenant_modules`, `module_dependencies` | `partial` | `tenant_config` | P0 | Enforcement harmoniseren, module dependency inspectie/visualisatie. Module uit via UI/direct URL/server action/API faalt. | `FG-MODULE-*`. | 11 |
| Plans en subscriptions | `plans`, `plan_modules`, `plan_limits`, `tenant_subscriptions` | `partial` | `tenant_config` | P0/P2 | Foundation bestaat: Starter/Professional/Enterprise, plan modules, plan limits, custom-role capability, custom domains, supportniveau, manual billing notes en subscriptionstatus als entitlement-bron. | Plan module seed, custom roles Professional+, custom domains Enterprise-only, `FG-OPS-003`. | 11/14/7 |
| Organization settings | `organization_settings`, branding, SMTP | `runtime-proof-open` | `tenant_config` | P2 | Branding preview bestaat voor backoffice, klantportaal, personeelsapp, email en PDF; Playwright bewijs blijft open. | Tenantbranding lekt niet, `FG-OPS-004`. | 14 |
