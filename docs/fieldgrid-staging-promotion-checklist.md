...
- [ ] De W00 migration-admin-eigendomsgate draait na alle databasebackfills maar vóór runtime-roleprovisioning/-verificatie en activatie en is exact aan de nieuwe release-SHA gebonden: normale/recovery-deploys gebruiken tenant A/B host- en ID-bindings; disposable rebuild gebruikt de afzonderlijke platform-only scope zonder permanente tenantbindings en bewijst nul tenants.
