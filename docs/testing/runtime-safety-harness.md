...
- `pnpm fieldgrid:runtime-safety:rls`: authenticated RLS checks using `SET LOCAL ROLE authenticated`, `row_security = on`, local JWT GUC shims, `aclexplode`, and `has_table_privilege`. In Phase B, `PUBLIC`, `anon`, and `authenticated` have no direct `assignment_personnel` table privileges. `service_role` keeps only SELECT/INSERT/UPDATE/DELETE. The RLS harness simulates historical broad ACL drift, reruns Phase-A.1 plus Phase-B migrations, proves direct table access is closed, proves service-role CRUD plus trigger invariants, and exercises assignments, tasks, extra work, photos, reports, report notes, attachments, material usage, objects, Tenant A/B isolation, wrong-user denial, inactive personnel denial, customer policy regressions, and the current schema invariant that one auth user cannot have multiple `personnel` rows across tenants because `personnel.user_id` is unique. The personnel JWT contract intentionally omits `tenant_id`; arbitrary or malformed `tenant_id` claims do not expand access because tenant binding is derived from active assigned personnel rows and same-tenant parent records.
- `pnpm fieldgrid:runtime-safety:previous-release-compatibility`: deployment compatibility checks for rollbackrelease `132e7d0705f0192d6ec4a28195f192850574447d`. This lane includes a static callsite audit, but its pass/fail signal also runs real PostgreSQL queries for the previous release server-action patterns and authenticated RLS queries for assignments, tasks, photos, and reports without a `tenant_id` JWT claim.
...
Fresh real Fieldgrid personnel-session evidence showed `role = authenticated`, `sub = present`, and `tenant_id_valid = false` before token expiry. The Phase-B migration therefore does not rely on a JWT tenant claim. It hardens `public.personnel_assigned_to_assignment(uuid)` as a minimal `SECURITY DEFINER` helper that derives tenant access from `auth.uid()`, an active assigned personnel row, and assignment/personnel same-tenant parents. The storage policy helper `public.personnel_can_access_assignment_storage(uuid, uuid)` applies the same database-derived tenant binding and, when a storage path contains a tenant id, requires that path tenant to equal the assignment tenant.

The local `storage.objects` tests are PostgreSQL policy evidence only. They do not prove live Supabase Storage object API behavior, signed URL behavior, or deployed provider policy configuration; that validation belongs after staging deployment.
