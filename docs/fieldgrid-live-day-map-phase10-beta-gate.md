...
- Provider fallback is veilig: zonder `GOOGLE_MAPS_SERVER_API_KEY` gebruikt de default provider de deterministische mockprovider. Zet `FIELDGRID_ROUTE_PROVIDER=google` alleen wanneer een geldige server-only Google key beschikbaar is.
...
3. Gebruik de Google productieprovider alleen met een geldige `GOOGLE_MAPS_SERVER_API_KEY`.
...
4. Als providerproblemen optreden, zet tijdelijk `FIELDGRID_ROUTE_PROVIDER=mock` of laat `GOOGLE_MAPS_SERVER_API_KEY` leeg zodat de mockprovider routecontexten kan berekenen.
