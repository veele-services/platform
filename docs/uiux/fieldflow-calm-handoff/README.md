...
De pakketvalidator borgt zowel de semantische inhoud van de machinecontracten als één geaggregeerde byte-digest van deze twaalf normatieve Markdown-bestanden. [contract-root.json](./manifests/contract-root.json) bindt daarnaast alle normatieve manifesten, productiebronnen, visuele/prototype-evidence, validator, hoofdtests, packagecommand, journeyhelper en de drie trusted workflows in één SHA-256. Nadat aan de gewone repository-branch-protection is voldaan, kopieert een beheerder die workflows byte-identiek naar default branch `main`, bevestigt dat Environment `fieldflow-calm-contract` alleen `main` toelaat en registreert daar de rootwaarde. De `pull_request_target`-jobs bewijzen vervolgens dat main-executor, protected base en kandidaat dezelfde workflowblob gebruiken. De contract-rootvalidator houdt kandidaatcode volledig inert; evidence- en baselinecode mag alleen in de hieronder begrensde producerzone draaien. Een contractwijziging vereist daardoor een afzonderlijke rootrotatie en, nadat de pending root extern is gezet, een generieke body- of titeledit als vertrouwde retrigger. Die edit heeft geen goedkeuringsbetekenis: het rootprotocol voegt geen revieweraantal, reviewerrol of reviewbodymarker toe bovenop de gewone branch protection. `pull_request_review` en automatisch “de hashes bijwerken” zijn geen geldige rootretrigger.

Runtime- en baselinebewijs gebruikt daarnaast een harde drielagengrens. Een disposable non-OIDC producer A is de enige job die kandidaatcommands mag uitvoeren; zijn raw artifact, receipt en nonce zijn onbetrouwbare transportdata. Een nieuwe non-OIDC validator B leidt immutable GitHub-bindingen zelf af, valideert alleen de exacte veilige reguliere-bestandsclosure met byte-identieke base-owned code/dependencies en publiceert een eigen run-/attempt-uniek validated artifact. Alleen signer C krijgt OIDC en attesteert uitsluitend B. De nog ontbrekende producer-runner moet kandidaatcode begrenzen met read-only bron, een afzonderlijke schrijfbare outputroot, een gescrubde omgeving zonder nonce/GitHub-credentials/secrets/file-commandpaden en volledige procesgroep-/cgroupterminatie vóór upload; tot die runner root-gebonden bestaat, falen beide producers bewust gesloten.
...
| Visuele auditankers      | 9 PNG's met afmeting en SHA-256; W00 promoveert 9 desktoprecords, W12 voegt 9 mobiele productierecords toe |
...
4. De capturelifecycle volgt exact `CONTRACTED → REFERENCE_READY → BASELINE_READY`. `CONTRACTED` bevat geen runtime-, font- of scenariobewijs; W00 eindigt pas op `REFERENCE_READY` met exact negen gereviewde desktoprecords; W12 promoveert pas naar `BASELINE_READY` door die negen records byte-identiek te behouden en exact negen mobiele productierecords toe te voegen. De gehashte Fieldflow CI-themefixture heeft pixelpariteit met de toegankelijk genormaliseerde desktopbaseline. De raw anchors zijn nooit een pixelgate. Kleur/font/radiusvarianten behouden de compositie; density wijzigt uitsluitend de vier benoemde geometry-assen en heeft per preset een eigen snapshot.
...
| [visual/capture-contract.json](./evidence/visual/capture-contract.json)             | Drietraps capturelifecycle: negen desktop-pixelrecords in W00, daarna negen mobiele productiecontracten voor layout, spacing, Axe, keyboard en touch in W12 |
...
4. Leg een actuele legacybaseline vast voordat UI-code wordt gewijzigd. W00 promoveert via een afzonderlijke tweefasen-PR van `CONTRACTED` naar `REFERENCE_READY`: gepinde runtime/fontbestanden en exact negen scenario-/contract-/runtimegebonden desktop-PNG's, ieder met vijf gevalideerde JSON-artefacten. W12 promoveert later van `REFERENCE_READY` naar `BASELINE_READY`: de negen desktoprecords en hun volgorde blijven byte-identiek en exact negen mobiele productiescreenshots met ieder acht gevalideerde JSON-artefacten (DOM, geometry, styles, setup, runtime-errors, Axe, keyboard en touch) worden als suffix toegevoegd. Iedere set heeft een HEAD-unieke, gehashte en offline verifieerbare Artifact Attestation-bundle plus onafhankelijke live `product-design`- en `visual-a11y`-reviews op exact HEAD; remote Actions-artifactretention is geen duurzame trustbron.
