...
5. Controleer dat de drie workflows byte-identiek op `main`, protected base en kandidaat staan, `github.ref`/`workflow_ref` exact `main` aanwijzen, Environment `fieldflow-calm-contract` alleen `main` toelaat, `contract-root.json#rootSha256` exact de beschermde Environment-variable is en de stabiele eindjob `Fieldflow Calm contract root` groen is. Stop bij ontbrekende bootstrap, rootdrift, een gewijzigde workflow of een skipped/cancelled/denied verificatiejob. Gebruik nooit `pull_request_review` voor rootrotatie; laat nadat de pending root extern op de bevroren kandidaat is gezet uitsluitend een gewone body- of titeledit het `pull_request_target: edited`-event retriggeren. Die edit is geen approval; het rootprotocol voegt niets toe aan de gewone branch-protectionreviews.
6. Leg de bestaande test-/visualbaseline van je scope vast en volg `capture-contract.json` letterlijk; maak nooit zelf vrije evidencevelden. `CONTRACTED` houdt runtime-image, fontfiles en scenario-evidence exact `null`. W00 produceert op historische capture-HEAD **C** exact negen canonieke desktop-pixelsets in de gecontracteerde volgorde en promoveert die pas via een latere smalle HEAD **D** naar `REFERENCE_READY`. W12 produceert later exact negen mobiele productie-evidencesets met elk DOM-, geometry-, computed-styles-, setup-, runtime-errors-, Axe-, keyboard- en touch-JSON en promoveert via een nieuwe C/D-cyclus naar `BASELINE_READY`. Daarbij blijven runtime/fontpayload, desktoprecords en desktopvolgorde byte-identiek en worden de mobiele records als geordend suffix toegevoegd. Iedere stap vereist inhoudelijke artefactvalidatie, live PR/run/job/reviewverificatie op de historische capture/evidence-HEAD en offline verificatie van een ingecheckte, gehashte Artifact Attestation-bundle van de al vertrouwde baselineworkflow. De baselineattestatie bevat exact manifest plus alle PNG's (10/19 subjects) en haar bundlepad bevat exact de capture-HEAD.
...
De gehashte v3-index volgt exact `acceptance.json#evidenceContract.indexSchema`: `subjectId`, `verification` en `headCommit` zijn exact het requirement en de historische implementation HEAD **C** in `evidence.commit`; ieder codepad is een werkelijk in de implementatie-PR gewijzigd bestand met blob-SHA-256 op **C**. `provenance.runtime` en `provenance.staging` zijn afzonderlijke post-upload records; ieder rapport bevat alleen de bijbehorende pre-upload projectie. Gebruik uitsluitend de vaste command-ID’s en exacte commandregels uit de catalogus, inclusief `--evidence-subject` en het bijbehorende gehashte rapportpad. Vrije shellcommands zoals `true`, `echo` of een los `pnpm test` zijn nooit bewijs.

Een runtime-/stagingclaim bevat exact één machineleesbaar, geattesteerd JSON-rapport. Controleer subject, implementation HEAD **C**, verification, de pre-upload provenanceprojectie, coverage, assertions, nul failed/skipped/notRun/manual, lege errorchannels en attachmenthashes. De workflow moet drie wegwerprunners scheiden: alleen A draait kandidaatcommands en zijn raw output/receipt/nonce is volledig onbetrouwbaar; schone non-OIDC job B leidt alle immutable bindings zelf af, accepteert alleen een exacte veilige closure zonder symlinks of hardlinks en draait uitsluitend byte-identieke base-owned validatie; alleen job C krijgt OIDC en attesteert B's attempt-unieke validated artifact. De toekomstig toe te voegen runner is containment-capabel: read-only kandidaatbron, dedicated writable output, geen nonce/GitHub-secret/credential/file-commandpad in de kandidaatprocessomgeving en geen levende kandidaatprocessen vóór raw transfer. Ontbreekt die runner, dan faalt de workflow gesloten. De validator moet ancestry, dezelfde workflowblob op main-executor/base/**C**, de live PR, Actions-run, check-suite en exacte attempt-job verifiëren zonder `run.pull_requests`. Attestationcontrole gebruikt de gehashte ingecheckte bundle met exacte main-cert-identity/issuer/source/signer/run/hosted-runnerbinding, één veilig uniek reportsubject en vertrouwt geen predicateclaims; API-/attestationfouten zijn rood. `artifactId`, de finale `artifactName` en `artifactDigest` zijn uitsluitend historische transportmetadata; raw-, validated- en finalenaam binden exact `runAttempt`, terwijl alleen de finalenaam in de index mag staan. Geen remote artifactretention vormt een toekomstige afhankelijkheid. De bundle staat exact op `outputs/fieldflow-calm/attestations/<subject>.<mode>.<head>.bundle.json`; rapporten/manifests bevatten zelf geen post-sign bundlevelden.
...
Promotie gebeurt altijd in twee fasen om commit-self-reference te vermijden. Fase 1 is de gemergede implementatie-/capture-PR op **C** met het echte bewijs. Fase 2 is een latere kandidaat-HEAD **D** die uitsluitend de gehashte bewijsclosure en de eerstvolgende lifecyclestate toevoegt. De protected-baseworkflow leest **D** alleen als inert Git-data, bewijst **C→D** ancestry en verifieert alle objecten op **C** live; **D** wordt nooit als de geteste implementation/capture HEAD voorgesteld. Bij gelijkblijvende state moet de volledige lifecyclepayload identiek blijven. De enige ketens zijn acceptance `CONTRACTED → IMPLEMENTED → VERIFIED_LOCAL → VERIFIED_STAGING → RELEASED`, risico `OPEN → MITIGATED → VERIFIED_LOCAL → VERIFIED_STAGING → CLOSED` en baseline `CONTRACTED → REFERENCE_READY → BASELINE_READY`. W00 mag alleen de eerste capturestap uitvoeren; W12 gebruikt een afzonderlijke C/D-cyclus voor de tweede stap en behoudt de negen desktoprecords byte-identiek als prefix. Gebruik geen top-level traceabilityvelden.
