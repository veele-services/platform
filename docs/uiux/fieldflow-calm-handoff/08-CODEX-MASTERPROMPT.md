...
5. Controleer dat de drie workflows byte-identiek op `main`, protected base en kandidaat staan, `github.ref`/`workflow_ref` exact `main` aanwijzen, Environment `fieldflow-calm-contract` alleen `main` toelaat, `contract-root.json#rootSha256` exact de beschermde Environment-variable is en de stabiele eindjob `Fieldflow Calm contract root` groen is. Stop bij ontbrekende bootstrap, rootdrift, een gewijzigde workflow of een skipped/cancelled/denied verificatiejob. Gebruik nooit `pull_request_review` voor rootrotatie; voeg na de drie reviews via een bevoegde echte body-edit exact één `FIELDFLOW-ROOT-RECHECK`-marker toe en laat uitsluitend het latere `pull_request_target: edited`-event valideren.
