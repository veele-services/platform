...
- interesse-“Selecteer en plan in” gebruikt `place` met `source.kind="interest"`; de Server Action lockt exact die tenant-owned response, vergelijkt haar version, vereist dat assignment en medewerker overeenkomen, zet haar atomair op `confirmed`, maakt/reactiveert de juiste `assigned`-koppeling, herberekent capaciteit met `resolveRequiredSlots`, plant de opdracht en schrijft één gezamenlijke receipt/audit/revision/outboxevent;
