...
| `FG-PA-GATE-HOST-FIRST`               | field-demo pilot host-first checks              | Platform engineering | `field-demo.staging.fieldgrid.nl` met `/`, `/klant`, `/personeel`         | Browser traces en wrong-host denial artifact | Host bepaalt tenantcontext en directe tenant-id routes lekken niet.                                                          |
