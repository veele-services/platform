...
| `FG-PA-GATE-HOST-FIRST` | Tenant A/B/Veele host-first checks | Platform engineering | `demo-a`, `demo-b`, `veele` met `/admin`, `/klant`, `/personeel` | Browser traces en wrong-host denial artifact | Host bepaalt tenantcontext en directe tenant-id routes lekken niet. |
...
| `FG-PA-GATE-LIFECYCLE` | Tenant lifecycle smoke | Platform engineering | `/platform/tenants/:tenantId` | Mutating smoke met cleanup en audit | Suspend/reactivate/archive/retry werkt op demo tenant en wordt teruggedraaid. |
