...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| Platform administration | 2 | 6 | 0 | 6 | 2 | Auth invite/reset side effects, ticket relationship integrity, lifecycle/domain/sector edge cases, notification delivery proof |
| Tenant backoffice | 7 | 8 | 0 | 6 | 0 | Global activity log, cross-tenant planning/auth actions, non-transactional report/invoice handoffs |
| Personnel PWA | 2 | 8 | 1 | 4 | 2 | Tenant-null reports, host drift, report submission event gap, offline durability |
| Customer PWA | 5 | 5 | 0 | 6 | 0 | Module bypass, document signing, expired quote decisions, draft invoice exposure, payment atomicity, synthetic notifications |
| Cross-surface | 2 | 5 | 1 | 6 | 0 | Event/outbox parity, tenant predicates, and financial state consistency |
...
| FG-SURFACE-PLATFORM-012 | Invite/reset auth side effects | unsafe | Auth password policy tests | Email-failure and existing-active account preservation tests | critical | Auth/platform |
...
| FG-SURFACE-BACKOFFICE-017 | Activity log | unsafe | No focused test | Tenant-filtered activity-log test | critical | Settings/audit |
| FG-SURFACE-BACKOFFICE-018 | Personnel password reset | unsafe | Auth/password tests | Cross-tenant personnel id denial | high | Auth/personnel |
...
| FG-SURFACE-PERSONNEL-014 | Tenant id on reports | unsafe | Report tenant-scope tests do not cover PWA insert | `reports.tenant_id = identity.tenantId` submit test | high | Personnel reporting |
...
| FG-SURFACE-CUSTOMER-012 | Module-gated direct access | unsafe | Portal acceptance/static tests | Direct action/route denial when finance/documents/reporting disabled | high | Entitlements/customer PWA |
| FG-SURFACE-CUSTOMER-013 | Document signed URLs | unsafe | Document storage/download tests | Reject non-tenant-bound `storage_path` before signing | high | Documents/storage |
| FG-SURFACE-CUSTOMER-014 | Notification preferences | partially connected | Preference action tests absent | Delivery filtering by customer portal preferences | medium | Notifications |
...
| FG-SURFACE-CROSS-008 | Invoice sent/paid to customer finance and closure | Backoffice/API | `invoices`, `payments`, assignment statuses | Customer invoices/payments, backoffice finance | unsafe | Mollie webhook and manual full-payment paths bypass `invoice_paid` event parity; webhook also lacks defensive tenant predicates |
...
| FG-SURFACE-CROSS-011 | Shared event recipient fanout | Workflow events | `domain_events`, customer/personnel notification rows | Customer/personnel notification centers | partially connected | Recipient lookups should enforce tenant membership defensively |
