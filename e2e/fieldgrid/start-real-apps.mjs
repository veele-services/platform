...
const defaultDatabaseUrl = 'postgresql://postgres:postgres@127.0.0.1:5432/fieldgrid_runtime_safety';
const configuredDatabaseUrl = process.env.DATABASE_URL ?? defaultDatabaseUrl;
const postgrestOrigin = process.env.FIELDGRID_E2E_POSTGREST_ORIGIN ?? `http://127.0.0.1:${ports.postgrest}`;
...
const localFixtureIdentities = {
  ...
  tenantBCustomer: { userId: '20000000-0000-4000-8000-000000000205', email: 'customer@tenant-b.runtime.fieldgrid.test' },
};
...
export function createJwt(sub, offsetSeconds = 0, lifetimeSeconds = jwtMaxLifetimeSeconds) {
  const identity = Object.values(localFixtureIdentities).find((candidate) => candidate.userId === sub);
  if (!identity) throw new Error('Cannot create a local E2E JWT for an unknown fixture identity.');
...
function spawnLogged(name, command, args, env) {
  ...
  const child = spawn(resolved.command, resolved.args, { env, stdio: ['ignore', 'pipe', 'pipe'], shell: false });
...
async function tcpReachable(host, port) {
  return new Promise((resolve) => {
    const socket = net.createConnection({ host, port, timeout: 1000 }, () => { socket.destroy(); resolve(true); });
...
async function proveDataPath() {
  const tenantACustomerJwt = createJwt(localFixtureIdentities.tenantACustomer.userId);
  const tenantBCustomerJwt = createJwt(localFixtureIdentities.tenantBCustomer.userId);
  ...
  const personnelTenantAAssignmentValue = personnelTenantAAssignment.ok ? await personnelTenantAAssignment.json() : null;
  const personnelTenantATenantBAssignmentValue = personnelTenantATenantBAssignment.ok ? await personnelTenantATenantBAssignment.json() : null;
  const personnelTenantBTenantAAssignmentValue = personnelTenantBTenantAAssignment.ok ? await personnelTenantBTenantAAssignment.json() : null;
  dataPathProof = {
    ...
    customerTenantAAllowedAssignmentCount: Array.isArray(customerTenantAAllowedRows) ? customerTenantAAllowedRows.length : 0,
    customerTenantADeniedTenantBAssignmentCount: Array.isArray(customerTenantADeniedTenantBRows) ? customerTenantADeniedTenantBRows.length : 0,
    customerTenantBDeniedTenantAAssignmentCount: Array.isArray(customerTenantBDeniedTenantARows) ? customerTenantBDeniedTenantARows.length : 0,
    personnelTenantAAssignmentAllowed: personnelTenantAAssignmentValue === true,
    personnelTenantATenantBAssignmentDenied: personnelTenantATenantBAssignmentValue === false,
    personnelTenantBTenantAAssignmentDenied: personnelTenantBTenantAAssignmentValue === false,
    invalidJwtStatus: invalid.status,
    serviceRoleBrowserBypassDetected: false,
  };
  const ok = dataPathProof.customerTenantAAllowedAssignmentCount === 1
...
async function probeApp(name, port, path, host, userId, acceptedStatuses) {
  try {
    ...
    return { name, ok: false, error: redact(error instanceof Error ? error.message : String(error)), checkedAt: new Date().toISOString() };
  }
}
...
async function probeLivenessApp(name, port, host, path = '/login') {
  try {
    ...
    return { name, ok: [200, 302, 307].includes(response.status), status: response.status, checkedAt: new Date().toISOString() };
  } catch (error) {
    return { name, ok: false, error: redact(error instanceof Error ? error.message : String(error)), checkedAt: new Date().toISOString() };
  }
}
...
async function authenticatedPreflight() {
  ...
  const checks = [];
  const targets = [
    { name: 'tenant-a-admin-backoffice', port: ports.backoffice, path: '/customers', host: 'tenant-a.runtime.fieldgrid.test', userId: localFixtureIdentities.tenantAAdmin.userId },
    { name: 'tenant-a-personnel', port: ports.personnel, path: '/personeel/opdrachten', host: 'tenant-a.runtime.fieldgrid.test', userId: localFixtureIdentities.tenantAPersonnel.userId },
    { name: 'tenant-a-customer', port: ports.customer, path: '/klant/opdrachten', host: 'tenant-a.runtime.fieldgrid.test', userId: localFixtureIdentities.tenantACustomer.userId },
  ];

  for (const target of targets) {
    const check = await probeApp(target.name, target.port, target.path, target.host, target.userId, [200]);
    checks.push(check);
    if (!check.ok) {
      latestPreflight = {
        ready: false,
  ...
  try {
    ...
    checks.push({ name: 'data-path-proof', ok: false, error: redact(error instanceof Error ? error.message : String(error)), checkedAt: new Date().toISOString() });
  }
  latestPreflight = { ready: checks.every((check) => check.ok), failedImmediately: false, checks, completedAt: new Date().toISOString() };
  await writeAtomicJson(preflightPath, latestPreflight);
  return latestPreflight;
}
    ...
    if (req.url === '/preflight') {
      try {
        ...
        latestPreflight = {
          ...
          checks: [{ name: 'preflight-crash', ok: false, error: redact(error instanceof Error ? error.message : String(error)), checkedAt: new Date().toISOString() }],
          completedAt: new Date().toISOString(),
        };
        await writeAtomicJson(preflightPath, latestPreflight);
...
export async function start() {
  ...
  startGateway();
  startOrchestrator();
  spawnLogged('backoffice', 'pnpm', ['--filter', '@workspace/backoffice', 'exec', 'next', 'dev', '-H', '127.0.0.1', '-p', String(ports.backoffice)], appEnv(ports.backoffice));
  spawnLogged('personnel', 'pnpm', ['--filter', '@workspace/personeel-pwa', 'exec', 'next', 'dev', '--turbopack', '-H', '0.0.0.0', '-p', String(ports.personnel)], appEnv(ports.personnel));
  spawnLogged('customer', 'pnpm', ['--filter', '@workspace/klant-pwa', 'exec', 'next', 'dev', '--turbopack', '-H', '0.0.0.0', '-p', String(ports.customer)], appEnv(ports.customer));
...
export async function stop() {
  ...
  await writeAtomicJson(statusPath, { ...latestLiveness, status: 'stopped', ports, artifactDir });
}
