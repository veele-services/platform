...
assert(offline.queueBeforeReconnect === 2, 'Offline queue must contain the required two-action mutation chain before reconnect', offline);
assert(offline.activeAttemptHeld === true, 'Deterministic active synchronization barrier was not observed', offline);
assert(offline.firstStillDurable === true, 'In-flight mutation identity disappeared from durable storage', offline);
...
assert(offline.clientAttemptCount === 3, 'Transient attempt, stable replay and dependent mutation were not observed', offline);
...
assert(Array.isArray(offline.sequentialMutationIdSha256) && offline.sequentialMutationIdSha256.length === 2, 'Sequential mutation identifiers are missing', offline);
assert(offline.sequentialMutationIdSha256.every((value) => /^[0-9a-f]{64}$/u.test(value)), 'Sequential mutation identifier hash is invalid', offline);
assert(typeof offline.entityStream === 'string' && offline.entityStream.length > 0, 'Offline entity stream is missing', offline);
assert(Number.isInteger(offline.initialVersion), 'Offline initial participant version is missing', offline);
assert(Array.isArray(offline.canonicalVersions) && offline.canonicalVersions.length === 2, 'Canonical version receipts are incomplete', offline);
assert(offline.canonicalVersions[0] === offline.initialVersion + 1, 'Lifecycle mutation did not return the next canonical version', offline);
assert(offline.canonicalVersions[1] === offline.canonicalVersions[0], 'Dependent mutation reused a stale rendered version', offline);
assert(offline.dependencyAdvanced === true, 'Dependency version advancement was not observed', offline);
assert(offline.transientFailure?.sqlState === '40001', 'Runtime database SQLSTATE was not preserved', offline);
assert(offline.transientFailure?.classification === 'transient' && offline.transientFailure.retryable === true, 'Runtime database failure was not classified as retryable', offline);
assert(offline.transientFailure?.retryAttempt === 1, 'Runtime database retry attempt is missing', offline);
assert(offline.transientFailure?.mutationIdSha256 === offline.sequentialMutationIdSha256[0], 'Transient retry changed the in-flight mutation identity', offline);
assert(/^[0-9a-f]{64}$/u.test(offline.transientFailure?.diagnosticIdSha256), 'Transient failure diagnostic identifier was not safely hashed', offline);
assert(offline.canonicalReceiptCount === 2, 'Offline mutation chain has no unique canonical server receipts', offline);
assert(offline.completedCanonicalReceiptCount === 2, 'Canonical offline receipts were not durably completed', offline);
assert(offline.serverMutationCount === 2, 'Offline mutation chain executed more or less than once per mutation on the server', offline);
