...
import {
  getCustomerFeatureRequestOptions,
  submitCustomerFeatureRequest,
...
async function submitAction(formData: FormData): Promise<void> {
  "use server";
  await submitCustomerFeatureRequest(formData);
}

export default async function CustomerFeatureRequestPage() {
  const options = await getCustomerFeatureRequestOptions();
          ...
          <form action={submitAction} className="mt-5 grid gap-4">
            ...
            >
              ...
              <input
                name="title"
                required
                placeholder="Bijvoorbeeld: Sneller rapportages downloaden"
                className="h-11 rounded-xl border border-slate-300 px-3 text-sm font-normal text-slate-900"
              />
            </label>
            <label
              className="grid gap-1 text-sm font-semibold"
            ...
            >
              ...
              <textarea
                ...
                required
                rows={6}
                placeholder="Beschrijf wat u mist en waarom dit helpt."
                className="rounded-xl border border-slate-300 px-3 py-2 text-sm font-normal leading-6 text-slate-900"
              />
            </label>
            <div className="grid gap-4 md:grid-cols-[180px_minmax(0,1fr)]">
              ...
              >
                ...
                <SelectAdapter
                  name="priority"
                  defaultValue="normal"
                  className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-normal text-slate-900"
                >
              ...
              >
                ...
                <SelectAdapter
                  name="moduleKeys"
                  multiple
                  className="min-h-32 rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-900"
                >
