    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-5 md:px-0">
      ...
      <section className="rounded-2xl border bg-white p-5 shadow-sm" style={{ borderColor: "var(--color-border)" }}>
        ...
        <div className="mt-5 grid gap-2 rounded-2xl border border-cyan-100 bg-cyan-50/70 p-4 text-sm text-slate-700">
          ...
          <p>Na indienen start uw wens als Nieuw. Daarna beoordeelt de backoffice deze als In overweging, In ontwikkeling of Afgerond.</p>
        </div>
        ...
        {!options.enabled ? (
          ...
          <form action={submitAction} className="mt-5 grid gap-4">
            <label className="grid gap-1 text-sm font-black" style={{ color: "var(--color-primary)" }}>
              Titel
              <input name="title" required placeholder="Bijvoorbeeld: Sneller rapportages downloaden" className="h-11 rounded-xl border border-slate-300 px-3 text-sm font-normal text-slate-900" />
            </label>
            <label className="grid gap-1 text-sm font-black" style={{ color: "var(--color-primary)" }}>
              Omschrijving
              <textarea name="description" required rows={6} placeholder="Beschrijf wat u mist en waarom dit helpt." className="rounded-xl border border-slate-300 px-3 py-2 text-sm font-normal leading-6 text-slate-900" />
            </label>
