...
export function RequestAssignmentForm({
  ...
  const initialSectorId =
    objects.find((object) => object.isActive && object.sectorId)?.sectorId ??
    sectors[0]?.id ??
    "";
  ...
  const [objectId, setObjectId] = useState(
    () =>
      objects.find(
        (object) => object.isActive && object.sectorId === initialSectorId,
      )?.id ?? "",
  );
  ...
  const filteredObjects = objects.filter(
    (object) => object.isActive && object.sectorId === sectorId,
  );
  ...
  function selectSector(nextSectorId: string) {
    ...
    setObjectId(
      objects.find(
        (object) => object.isActive && object.sectorId === nextSectorId,
      )?.id ?? "",
    );
  }
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center py-10 text-center">
        ...
        >
          Uw aanvraag wordt beoordeeld. Daarna ontvangt u een offerte ter
          akkoord.
        </p>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                  {
                    objects.filter(
                      (object) =>
                        object.isActive && object.sectorId === sector.id,
                    ).length
                  }{" "}
                  object(en)
                </span>
      ...
      <div>
        ...
        {selectedObject && (
          <div
            className="mt-3 rounded-2xl border bg-slate-50 p-3 text-xs"
            style={{ borderColor: "var(--color-border)" }}
          >
            <div
