...
function formatDate(iso: string | null) {
  ...
  return new Date(iso.includes("T") ? iso : `${iso}T00:00:00`).toLocaleDateString("nl-NL", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
...
export default async function ObjectDetailPage({ params }: Props) {
  ...
  const [object, sectors, assignments, reports, documents, tickets] = await Promise.all([
    getMyObject(id),
  ...
  const objectAssignments = assignments.filter((assignment) => assignment.objectId === object.id);
  const objectReports = reports.filter((report) => report.objectId === object.id);
  const objectDocuments = documents.filter((document) => document.objectId === object.id || document.entityId === object.id);
  const objectTickets = tickets.filter((ticket) => {
    const haystack = `${ticket.subject} ${ticket.lastMessagePreview ?? ""}`.toLowerCase();
    return haystack.includes(object.code.toLowerCase()) || haystack.includes(object.name.toLowerCase());
  });
    ...
    >
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 truncate text-2xl font-black" style={{ color: "var(--color-primary)" }}>
              {object.name}
            </h1>
            {addressLine ? (
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <DetailItem label="Opdrachten" value={`${objectAssignments.length}`} />
          <DetailItem label="Documenten" value={`${objectDocuments.length}`} />
            ...
            {objectAssignments.slice(0, 8).map((assignment) => (
              ...
              >
                <span className="min-w-0">
                  <span className="block truncate font-mono text-xs font-black" style={{ color: "var(--color-accent)" }}>
                    {assignment.code}
                  </span>
                ...
                <span className="text-xs font-bold" style={{ color: "var(--color-secondary)" }}>
                  {STATUS_LABEL[assignment.status] ?? assignment.status} - {formatDate(assignment.scheduledDate ?? assignment.createdAt)}
                </span>
              </Link>
            ))}
          </div>
        ) : (
          <EmptyState title="Nog geen opdrachten" description="Nieuwe of geplande opdrachten voor dit object verschijnen hier." />
        )}
      </Section>
      ...
      >
        {objectDocuments.length > 0 ? (
              ...
              >
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 block truncate text-xs font-semibold" style={{ color: "var(--color-muted-fg)" }}>
                    {document.assignmentCode ? `Opdracht ${document.assignmentCode}` : "Objectdocument"} - {formatDate(document.createdAt)}
                  </span>
                </span>
          ...
          <EmptyState title="Geen documenten" description="Object- en opdrachtdocumenten verschijnen hier zodra ze gedeeld zijn." />
        )}
      </Section>
            ...
            {objectReports.slice(0, 8).map((report) => (
              ...
              >
                ...
                <span className="mt-1 block text-xs font-semibold" style={{ color: "var(--color-secondary)" }}>
                  {formatDate(report.submittedAt)} - {report.hoursWorked ? `${Number.parseFloat(report.hoursWorked).toLocaleString("nl-NL")} uur` : "Geen uren"}
                </span>
              </Link>
            ))}
          </div>
        ) : (
          <EmptyState title="Geen rapportages" description="Goedgekeurde rapportages voor dit object verschijnen hier." />
        )}
      </Section>
      ...
      >
        ...
        {objectTickets.length > 0 ? (
          ...
          <EmptyState title="Geen objecttickets" description="Maak een supportvraag aan wanneer er iets speelt rond dit object." />
        )}
      </Section>
