        ...
        >
          {release.contentHtml ? (
            <KnowledgebaseContentRenderer
              html={release.contentHtml}
              mediaBasePath="/releases/media"
            />
          ) : (
            <p className="text-sm leading-7 text-slate-600">
              {release.contentText ?? release.summary}
            </p>
          )}
        </div>
      </article>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {release.media.map((item) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="overflow-hidden rounded-2xl border border-slate-200 text-sm"
              >
                {item.mediaType === "image" ? (
                  <img
                    src={`/releases/media/${item.id}`}
