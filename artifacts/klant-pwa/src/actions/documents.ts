...
type CustomerVisibleDocumentEntityType = Extract<DocumentEntityType, "customer" | "object" | "assignment">;
...
export async function getMyDocuments(): Promise<CustomerDocument[]> {
  ...
  const [customerDocuments, objectDocuments, assignmentDocuments] = await Promise.all([
    db
      .select({
    ...
    db
      ...
      .from(documentsTable)
      .innerJoin(assignmentsTable, eq(assignmentsTable.id, documentsTable.entityId))
  ...
  return [...customerDocuments, ...objectDocuments, ...assignmentDocuments]
    .filter((row): row is DocumentRow =>
      row.entityType === "customer" || row.entityType === "object" || row.entityType === "assignment",
    )
...
async function canAccessDocumentEntity(input: {
  ...
  if (entityType === "customer") {
    const [row] = await db
      ...
      .from(customersTable)
      .where(and(eq(customersTable.id, entityId), eq(customersTable.id, customerId), eq(customersTable.tenantId, tenantId)))
      .limit(1);
    return Boolean(row);
  }
  ...
  if (entityType === "object") {
    const [row] = await db
      ...
      .from(objectsTable)
      .where(and(eq(objectsTable.id, entityId), eq(objectsTable.customerId, customerId), eq(objectsTable.tenantId, tenantId)))
      .limit(1);
    return Boolean(row);
  }
  ...
  if (entityType === "assignment") {
    const [row] = await db
      ...
      .where(and(eq(assignmentsTable.id, entityId), eq(assignmentsTable.customerId, customerId), eq(assignmentsTable.tenantId, tenantId)))
      .limit(1);
    return Boolean(row);
  }
  ...
  const [doc] = await db
    ...
    .where(
      and(
        ...
        inArray(documentsTable.entityType, ["customer", "object", "assignment"]),
      ),
    )
    .limit(1);
