    ...
    await db.transaction(async (tx) => {
      ...
      if (!payment) {
        const [batch] = await tx
          ...
          .where(
            eq(customerPaymentBatchesTable.molliePaymentId, molliePaymentId),
          )
          .limit(1);

        if (!batch) {
          req.log.warn(
            { molliePaymentReference },
            "No local payment or payment batch found for Mollie payment ID",
          );
          return;
        }
        ...
        await tx
          ...
          .set({
            ...
            paidAmountCents:
              mollieStatus === "paid" ? batch.amountCents : undefined,
        ...
        if (mollieStatus === "paid") {
          ...
          for (const item of items) {
            ...
            if (!invoice || !invoice.tenantId || invoice.status !== "sent")
              continue;
      ...
      if (!payment.invoiceId) {
        req.log.warn(
          { molliePaymentReference },
          "Mollie payment has no invoice or collection source",
        );
        return;
      }
