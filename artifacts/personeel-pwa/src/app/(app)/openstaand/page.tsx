...
type OpenAssignmentFilterStatus = "all" | "available" | "invited" | "responded" | "unavailable";
type OpenAssignmentPriorityFilter = "all" | "urgent" | "high" | "normal" | "low";
...
const STATUS_FILTERS: Array<{ value: OpenAssignmentFilterStatus; label: string }> = [
...
const PRIORITY_FILTERS: Array<{ value: OpenAssignmentPriorityFilter; label: string }> = [
...
function normalizeStatus(value: string | undefined): OpenAssignmentFilterStatus {
  return STATUS_FILTERS.some((option) => option.value === value)
    ? value as OpenAssignmentFilterStatus
    : "all";
}

function normalizePriority(value: string | undefined): OpenAssignmentPriorityFilter {
...
function responseState(assignment: OpenAssignment): OpenAssignmentFilterStatus {
  ...
  if (
    ...
    ["interested", "question", "selected", "reserve", "confirmed"].includes(assignment.interestStatus ?? "")
  ) {
    return "responded";
  }
...
function matchesOpenAssignmentQuery(assignment: OpenAssignment, query: string): boolean {
...
function filterOpenAssignments({
  ...
  return assignments.filter((assignment) => {
    const priorityMatches = priority === "all" || assignment.priority === priority;
    const statusMatches = status === "all" || responseState(assignment) === status;
    return priorityMatches && statusMatches && matchesOpenAssignmentQuery(assignment, query);
  });
}
...
function OpenAssignmentsCommandBar({
  ...
  const activeFilters = [
    query ? { label: `Zoeken: ${query}`, href: openAssignmentsHref({ q: "", status, priority }) } : null,
    status !== "all"
      ? {
          label: STATUS_FILTERS.find((option) => option.value === status)?.label ?? "Status",
          href: openAssignmentsHref({ q: query, status: "all", priority }),
        }
      : null,
    priority !== "all"
      ? {
          label: PRIORITY_FILTERS.find((option) => option.value === priority)?.label ?? "Prioriteit",
  ...
  return (
    <section className="rounded-[22px] border bg-white p-3 shadow-sm md:p-4" style={{ borderColor: "var(--color-border)" }}>
      <form action="/openstaand" className="grid gap-2 md:grid-cols-[minmax(0,1fr)_11rem_12rem_auto] md:items-end">
        <div>
          <label htmlFor="open-assignment-search" className="mb-1.5 block text-[11px] font-black uppercase tracking-wide" style={{ color: "var(--color-secondary)" }}>
            Zoeken
          </label>
          <div className="flex h-11 items-center gap-2 rounded-2xl border px-3" style={{ borderColor: "var(--color-border)" }}>
            <Search size={16} strokeWidth={2.35} style={{ color: "var(--color-muted-fg)" }} />
            <input
        ...
        <div>
          <label htmlFor="open-assignment-status" className="mb-1.5 block text-[11px] font-black uppercase tracking-wide" style={{ color: "var(--color-secondary)" }}>
            Status
          </label>
          <select
        ...
        <div>
          <label htmlFor="open-assignment-priority" className="mb-1.5 block text-[11px] font-black uppercase tracking-wide" style={{ color: "var(--color-secondary)" }}>
            Prioriteit
          </label>
          <select
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <p className="text-[12px] font-bold" style={{ color: "var(--color-secondary)" }}>
        ...
        {activeFilters.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            ...
            <a href="/openstaand" className="rounded-full px-2.5 py-1 text-[11px] font-black" style={{ color: "var(--color-secondary)" }}>
              Wissen
            </a>
          </div>
        ) : null}
...
export default async function OpenstaandePage({ searchParams }: Props) {
  ...
  const visibleAssignments = filterOpenAssignments({ assignments, query, status, priority });
  ...
  return (
    ...
    >
      ...
      {assignments.length === 0 ? (
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-1 text-xs" style={{ color: "var(--color-secondary)" }}>
            Er zijn momenteel geen opdrachten beschikbaar waarvoor je in aanmerking komt.
          </p>
        </div>
      ) : null}
      ...
      {assignments.length > 0 && visibleAssignments.length === 0 ? (
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
          <Search size={32} className="mx-auto mb-3" style={{ color: "var(--color-muted-fg)" }} />
          <p className="text-sm font-black" style={{ color: "var(--color-primary)" }}>
            Geen diensten gevonden
          </p>
          <p className="mt-1 text-xs" style={{ color: "var(--color-secondary)" }}>
            Pas je zoekopdracht of filters aan om de beschikbare diensten opnieuw te bekijken.
          </p>
        </div>
      ) : null}
