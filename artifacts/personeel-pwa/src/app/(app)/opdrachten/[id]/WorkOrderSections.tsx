...
export function AssignmentQuestionCard({
  assignment,
}: {
...
export function CustomerInfoCard({
  ...
  const companyName =
    ...
    "Niet bekend";
  const contactName =
    assignment.contactName || assignment.customerName || "Niet bekend";
  const postalCity =
...
export function TaskChecklistCard({
  ...
  const [tasks, setTasks] = useState(() =>
    [...assignment.tasks].sort((a, b) => a.sortOrder - b.sortOrder),
  );
  const [error, setError] = useState<string | null>(null);
  ...
  function toggleTask(taskId: string, completed: boolean) {
    ...
    startTransition(async () => {
      const result = await setAssignmentTaskCompletion(
        assignment.id,
        taskId,
        completed,
        {
...
function ExtraWorkSubline({ item }: { item: ExtraWorkItem }) {
  ...
  if (hours > 0 && price > 0) {
    return (
      <>
        {formatQuantity(hours)} uur x {formatMoney(price)}
      </>
    );
  }
  if (price > 0) {
...
export function ExtraWorkSummaryCard({
  ...
  return (
    ...
    >
      <Link
        href={`/opdrachten/${assignmentId}/meerwerk`}
...
export function MaterialSummaryCard({
  ...
  return (
    ...
    >
      <Link
        href={`/opdrachten/${assignmentId}/materiaal`}
        className="mb-3 flex items-center justify-between gap-3"
      >
        <h2
          className="text-[19px] font-black leading-tight"
