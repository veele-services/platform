...
function SummaryAccordion({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#E9FBF8]"
...
function TaskRows({
  ...
  const completedCount =
    mode === "completed" ? tasks.length : getTaskCompletionCount(assignment);

  if (tasks.length === 0) {
    return (
      <p className="text-[13px]" style={{ color: "var(--color-secondary)" }}>
        Geen werkzaamheden gekoppeld.
      </p>
    );
  }
...
function ExtraWorkRows({ items }: { items: ExtraWorkItem[] }) {
  ...
  const total = items.reduce(
    (sum, item) => sum + calculateExtraWorkLineTotal(item),
    0,
  );
      ...
      {items.map((item) => (
        <div key={item.id} className="grid grid-cols-[1fr_auto] gap-3">
          <div className="min-w-0">
            ...
            >
              {item.hours
                ? `${formatQuantity(parseNumber(item.hours))} uur`
                : "1 item"}
            </p>
          </div>
          <span
...
function MaterialRows({ items }: { items: MaterialUsageItem[] }) {
  ...
  const total = items.reduce(
    (sum, item) => sum + calculateMaterialLineTotal(item),
    0,
  );
...
function ReportNoteRows({ notes }: { notes: ReportNote[] }) {
  if (notes.length === 0) {
    return (
      <p className="text-[13px]" style={{ color: "var(--color-secondary)" }}>
        Geen rapportagenotities vastgelegd.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {notes.map((note) => (
        ...
        >
          <div
            className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] font-bold"
            style={{ color: "var(--color-secondary)" }}
          >
          ...
          <p
            className="mt-1 whitespace-pre-line text-[13px] font-semibold leading-5"
            style={{ color: "var(--color-primary)" }}
          >
            {note.body}
          </p>
          {note.attachments.length > 0 ? (
            <p
              ...
              style={{ color: "var(--color-accent)" }}
            >
              {note.attachments.length} bijlage
              {note.attachments.length === 1 ? "" : "n"}
            </p>
          ) : null}
        </article>
      ))}
  ...
  return (
    ...
    >
      ...
      <p
        ...
        style={{ color: "var(--color-secondary)" }}
      >
        De klant gaat akkoord met de uitgevoerde werkzaamheden, eventueel
        meerwerk, gebruikte materialen en rapportage.
      </p>
      <div
        className={`mt-3 rounded-2xl border bg-white ${required ? "" : "opacity-55"}`}
        style={{ borderColor: "var(--color-border)" }}
      >
        {required ? (
          <canvas
          ...
          <div
            className="flex h-28 items-center justify-center px-4 text-center text-[13px] font-semibold"
            style={{ color: "var(--color-secondary)" }}
          >
...
export function CompletionSummary({
  ...
  const completedTaskCount =
    mode === "completed" ? taskCount : getTaskCompletionCount(assignment);
  ...
  function handleSubmit() {
    ...
    if (!canSubmit) {
      setError(
        "Deze werkbon moet eerst gestart zijn voordat je hem kunt afronden of afmelden.",
      );
      return;
    }
    if (
      mode === "completed" &&
      assignment.customerSignatureRequired &&
      !signatureDataUrl
    ) {
      setError(
        "Laat de klant eerst tekenen om de werkbon definitief gereed te melden.",
      );
      return;
    }
    ...
    if (isOfflineNow()) {
      ...
      setNotice(
        "Bevestiging is offline opgeslagen en wordt automatisch gesynchroniseerd.",
      );
      window.setTimeout(() => {
  ...
  return (
    <section className="space-y-3 px-3.5 pb-32 pt-4">
      ...
      <SummaryAccordion
        ...
        rightLabel={
          reportNotes.length > 0 ? `${reportNotes.length}` : undefined
        }
      >
      ...
      {mode === "completed" ? (
        <SignaturePad
          required={assignment.customerSignatureRequired}
