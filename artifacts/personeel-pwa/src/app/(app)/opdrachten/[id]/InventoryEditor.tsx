...
export function InventoryEditor({
  ...
  const selectedItem =
    catalog.find((item) => item.id === form.inventoryItemId) ?? null;
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      setForm(createEmptyForm(catalog));
      setNotice(
        "Inventaris is aan de werkbon gekoppeld en wacht op beoordeling.",
      );
    });
  }
          ...
          {items.length > 0 ? (
            ...
            >
              ...
              <p
                ...
                style={{ color: "var(--color-secondary)" }}
              >
                Koppel gebruikte, verhuurde of defect geconstateerde inventaris
                aan deze werkbon.
              </p>
            </div>
          )}
            ...
            <label className="block">
              ...
              <SelectAdapter
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    usageType: event.target.value as InventoryUsageType,
                  }))
                }
                className="w-full rounded-2xl border px-3 py-3 text-[14px] font-semibold outline-none"
                style={{
