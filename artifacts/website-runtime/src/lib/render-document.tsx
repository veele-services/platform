import type { ManagedWebsiteResolution } from "@workspace/db/website-public-runtime";
import type {
  WebsiteAction,
...
function actionHref(
  ...
  switch (action.kind) {
    case "page":
      return (
        snapshot.pages.find((page) => page.id === action.pageId)?.path ?? null
      );
    case "path":
      return action.path;
    case "external":
...
function ActionLink({
  ...
  const href = actionHref(action, snapshot);
  if (!href) return null;
...
export function ManagedWebsiteView({
  ...
  return (
    <div
      ...
      data-delivery-revision={resolution.deliveryRevision}
      style={style}
    >
      ...
      <header className="site-header">
        <div className="container header-inner">
          ...
          <nav aria-label="Hoofdnavigatie">
            <Navigation snapshot={snapshot} location="header" />
          </nav>
        </div>
      </header>
      <main id="inhoud">
        ...
        {page.sections.map((section) => {
          ...
          return (
            <React.Fragment key={section.id}>
              {renderSection(section, snapshot, firstHero)}
            </React.Fragment>
          );
        })}
      </main>
      <footer className="site-footer">
        <div className="container footer-grid">
          ...
          <nav aria-label="Voettekst">
            <Navigation snapshot={snapshot} location="footer_primary" />
          </nav>
          <nav aria-label="Juridische links">
            <Navigation snapshot={snapshot} location="footer_legal" />
          </nav>
        </div>
      </footer>
    </div>
