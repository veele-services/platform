...
async function safeDossierOptional<T>(
  loader: () => Promise<T>,
  fallback: T,
): Promise<T> {
  try {
...
export default async function PersonnelDetailPage({ params }: Props) {
  ...
  const typeLabel = person.personnelType
    ? (PERSONNEL_TYPE_LABELS[person.personnelType as PersonnelType] ??
    ...
    <TenantPageShell size="default">
      <TenantDetailHeader
        ...
        meta={[
          ...
          {
            label: "Branch/regio",
            value: person.region ?? "Geen primaire branch/regio",
          },
          {
            label: "Standaard vervoer",
      ...
      {!dossier && (
        ...
        >
          Dossierstatus is tijdelijk niet beschikbaar; het personeelsprofiel
          blijft bruikbaar.
        </div>
      )}
      {(canManageDossiers ||
        ...
        !dossierWorkspace && (
          ...
          >
            ...
            profielonderdelen blijven beschikbaar.
          </div>
        )}
      ...
      <TenantDetailSectionNav
        items={[
          ...
          ...(canReadDocuments
            ...
            : []),
          ...(dossierWorkspace
            ? [{ label: "Dossier 360", href: "#dossier-360" }]
            : []),
        ]}
      />
      ...
      >
        ...
        <div
          id="profile"
          className="grid scroll-mt-24 grid-cols-1 gap-5 lg:grid-cols-3"
        >
          {/* ── Left column ──────────────────────────────── */}
                ...
                {qualificationLinks.length > 0 ? (
                  <>
                    <QualificationLinkSection
                      label="Certificaten"
                      links={qualificationLinks.filter(
                        (link) => link.qualificationType === "certificate",
                      )}
                      color="#0A7E7A"
                      bg="#E0FAFB"
                    />
                    <QualificationLinkSection
                      label="Diploma's"
                      links={qualificationLinks.filter(
                        (link) => link.qualificationType === "diploma",
                      )}
                      color="#5A3B9A"
                      bg="#F0EBFF"
                    />
                    <QualificationLinkSection
                      label="Kennis"
                      links={qualificationLinks.filter(
                        (link) => link.qualificationType === "knowledge",
                      )}
                      color="#7C5A00"
                      bg="#FFF7E0"
                    />
                  ...
                  <>
                    <QualSection
                      label="Certificaten"
                      tags={person.certificates.map((c) => c.name)}
                      color="#0A7E7A"
                      bg="#E0FAFB"
                    />
                    <QualSection
                      label="Diploma's"
                      tags={person.diplomas}
                      color="#5A3B9A"
                      bg="#F0EBFF"
                    />
                    <QualSection
                      label="Kennis"
                      tags={person.knowledge}
                      color="#7C5A00"
                      bg="#FFF7E0"
                    />
                  </>
            ...
            return (
              <div key={link.id} className="rounded-lg border p-2.5">
                ...
                <p className="mt-1 text-xs" style={{ color: "#64748B" }}>
                  ...
                  {link.issuedAt
                    ? ` - afgegeven ${new Date(`${link.issuedAt}T00:00:00`).toLocaleDateString("nl-NL")}`
                    : ""}
                  {link.expiresAt
                    ? ` - verloopt ${new Date(`${link.expiresAt}T00:00:00`).toLocaleDateString("nl-NL")}`
                    : ""}
                </p>
              </div>
            );
