...
import { ChevronRight, Inbox, MessageSquare, Search, SlidersHorizontal } from "lucide-react";

import { listTickets, type BackofficeTicketListItem } from "@/app/actions/tickets";
...
function ticketsHref(params: { search?: string; status?: string; source?: string }) {
  const next = new URLSearchParams();
  if (params.search) next.set("search", params.search);
  if (params.status && params.status !== "all") next.set("status", params.status);
  if (params.source && params.source !== "all") next.set("source", params.source);
  const query = next.toString();
  return query ? `/tickets?${query}` : "/tickets";
}

function ticketStatusLabel(value: string) {
  return TICKET_STATUS_OPTIONS.find((option) => option.value === value)?.label ?? value;
}

function ticketSourceLabel(value: string) {
  return TICKET_KIND_OPTIONS.find((option) => option.value === value)?.label ?? value;
}
...
export default async function TicketsPage({ searchParams }: Props) {
  ...
  const activeFilterCount = [search, status !== "all" ? status : "", source !== "all" ? source : ""].filter(Boolean).length;
    ...
    <TenantPageShell size="wide">
      ...
      <TenantCommandBar
        ...
        activeFilters={
          <TenantActiveFilters
            filters={[
              ...(search ? [{ id: "search", label: "Zoek", value: search, href: ticketsHref({ status, source }) }] : []),
              ...(status !== "all" ? [{ id: "status", label: "Status", value: ticketStatusLabel(status), href: ticketsHref({ search, source }) }] : []),
              ...(source !== "all" ? [{ id: "source", label: "Bron", value: ticketSourceLabel(source), href: ticketsHref({ search, status }) }] : []),
            ]}
        ...
        >
          <div className="space-y-2 p-3">
            {result.rows.length > 0 ? (
              result.rows.map((ticket) => <TicketInboxCard key={`${ticket.kind}-${ticket.id}`} ticket={ticket} />)
            ) : (
              <div className="rounded-lg border border-dashed border-border px-4 py-14 text-center">
                <Inbox className="mx-auto h-8 w-8 text-muted-foreground" />
                <p className="mt-3 text-sm font-semibold text-foreground">Geen tickets gevonden</p>
    ...
    <TenantFilterDrawer
      ...
      footer={
        <>
          ...
          <Button type="submit" form="ticket-filter-form">Toepassen</Button>
        </>
      }
    >
      <form id="ticket-filter-form" action={backofficePath("/tickets")} className="space-y-4">
        ...
        <label className="grid gap-1.5 text-sm font-medium">
          Bron
          <select name="source" defaultValue={source} className="h-10 rounded-md border border-input bg-background px-3 text-sm">
      ...
      <div className="flex items-start gap-4">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-start gap-2">
            <span className="min-w-0 flex-1">
              <span className="line-clamp-1 text-sm font-semibold text-foreground">{ticket.subject}</span>
...
function TicketPreviewPanel({ ticket }: { ticket: BackofficeTicketListItem | null }) {
  return (
    <TenantWorkbenchPanel
      title="Preview"
      ...
      {ticket ? (
        <div className="space-y-4 p-4">
          <div className="space-y-2">
            ...
            <h2 className="text-lg font-semibold leading-7 text-foreground">{ticket.subject}</h2>
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Laatste bericht</p>
            <p className="mt-2 whitespace-pre-line text-sm leading-6 text-foreground">
              {ticket.lastMessagePreview ?? "Nog geen berichtinhoud"}
            </p>
            <p className="mt-3 text-xs font-medium text-muted-foreground">{formatDate(ticket.lastMessageAt)}</p>
          </div>
          ...
          <Button asChild className="w-full">
            <Link href={`/tickets/${ticket.kind}/${ticket.id}`}>Open gesprek</Link>
          </Button>
        </div>
      ) : (
