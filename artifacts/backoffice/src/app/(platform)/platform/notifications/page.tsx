...
function TemplateGuide({
  ...
  dashboard: PlatformNotificationCenter;
}) {
  return (
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Standaardteksten voor onderhoud, storing, onboarding, domein-DNS en
          abonnementswaarschuwingen.
        </p>
      </div>
...
function TenantOwnerRow({
  owner,
}: {
  owner: PlatformNotificationTenantRecipientPreview;
}) {
  return (
    <label className="grid gap-3 rounded border border-slate-200 bg-white p-3 sm:grid-cols-[auto_minmax(0,1fr)_170px] sm:items-center">
      ...
      <span className="min-w-0">
        ...
        <span className="block truncate text-xs text-slate-500">
          {owner.tenantSlug} - {owner.ownerEmail}
        </span>
      </span>
      <span
        className={`w-fit rounded border px-2 py-1 text-xs font-medium ${readinessChipClass(owner.readinessStatus)}`}
      >
  ...
  return (
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Selecteer altijd een doelgroep uit platformdata; losse
          ontvangerlijsten zijn bewust niet toegestaan.
        </p>
      </div>
      ...
      <form action={createDispatchFormAction} className="grid gap-5">
        <div className="grid gap-3 lg:grid-cols-[220px_260px_1fr]">
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <SelectAdapter
              name="templateKey"
              defaultValue={defaultTemplate.key}
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <input
              name="title"
              defaultValue={defaultTemplate.title}
              maxLength={180}
        ...
        <label className="grid gap-1 text-sm font-medium text-slate-700">
          ...
          <textarea
            name="body"
            defaultValue={defaultTemplate.body}
            rows={4}
            maxLength={5000}
        ...
        <section className="rounded border border-slate-200 bg-slate-50 p-4">
          <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Gebruik deze selectie alleen bij doelgroep “Specifieke tenant
                owners”.
              </p>
            </div>
          ...
          <div className="grid max-h-[420px] gap-2 overflow-y-auto pr-1">
            {dashboard.tenantOwners.map((owner) => (
              <TenantOwnerRow
                key={`${owner.tenantId}:${owner.ownerEmail}`}
                owner={owner}
              />
            ))}
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            className="inline-flex h-10 items-center gap-2 rounded bg-slate-950 px-4 text-sm font-semibold text-white transition hover:bg-slate-800"
          >
  ...
  return (
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Aantallen en concrete tenant-owner targets worden server-side uit
          platformdata opgebouwd.
        </p>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <div className="rounded border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Actieve platform users, op user-id snapshot.
          </p>
        </div>
        <div className="rounded border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm font-semibold text-slate-950">
          ...
          <p className="mt-1 text-2xl font-semibold text-slate-950">
            {dashboard.readinessIssueOwners.length}
          </p>
          <p className="mt-2 text-xs text-slate-500">
            Tenant owners met blocked of warning readiness.
          </p>
        </div>
        <div className="rounded border border-slate-200 bg-slate-50 p-4">
          <p className="text-sm font-semibold text-slate-950">Tenant owners</p>
          <p className="mt-1 text-2xl font-semibold text-slate-950">
            {dashboard.tenantOwners.length}
          </p>
          <p className="mt-2 text-xs text-slate-500">
            Alleen owner invites met pending, sent of accepted status.
          </p>
        </div>
      </div>
            ...
            {dashboard.plans.map((plan) => (
              ...
              >
                ...
                <span className="text-slate-500">
                  {plan.recipientCount} owner ontvanger(s)
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="grid max-h-[290px] gap-2 overflow-y-auto pr-1">
            {dashboard.modules.map((module) => (
              ...
              >
                ...
                <span className="text-slate-500">
                  {module.recipientCount} owner ontvanger(s)
                </span>
              </div>
            ))}
          </div>
...
function DispatchCard({
  ...
  dispatch: PlatformNotificationDispatchRow;
}) {
  return (
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded border px-2 py-1 text-xs font-medium ${statusChipClass(dispatch.status)}`}
            >
              {dispatch.status}
            </span>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {dispatch.scheduleType === "scheduled"
              ? `Gepland ${formatDate(dispatch.scheduledAt)}`
              : `Klaargezet ${formatDate(dispatch.queuedAt)}`}
          </p>
        </div>
  ...
  return (
    <main className="platform-page mx-auto flex w-full max-w-7xl flex-col gap-5 p-4 md:p-6">
      <div className="flex flex-col gap-2 border-b border-slate-200 pb-4">
        ...
        <div>
          <p className="text-sm font-medium text-slate-500">
            Fieldgrid communicatie
          </p>
          <h1 className="text-3xl font-semibold tracking-normal text-slate-950">
            Meldingen
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Gerichte platformmeldingen naar platformgebruikers en tenant owners,
            met zichtbare selectie en auditbare dispatch.
          </p>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <Stat
          label="Dispatches"
          value={dashboard.stats.totalDispatches}
          icon={Megaphone}
        />
        ...
        <Stat
          label="Ontvangers"
          value={dashboard.stats.recipients}
          icon={UsersRound}
        />
        <Stat
          label="Tenant owners"
          value={dashboard.stats.tenantOwners}
          icon={Mail}
        />
        <Stat
          label="Readiness issues"
          value={dashboard.stats.readinessIssues}
          icon={Bell}
        />
      </div>
      ...
      <section className="grid gap-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Laatste 50 platform-notificaties met doelgroep, planning, kanalen
              en ontvangersnapshot.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs text-slate-500">
            <span className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-2 py-1">
              <MessageSquareText className="h-3 w-3" /> In-app
            </span>
            <span className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-2 py-1">
              <Mail className="h-3 w-3" /> E-mail
            </span>
            <span className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-2 py-1">
              <Smartphone className="h-3 w-3" /> Push later
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              Ontvangers worden per dispatch server-side gematerialiseerd met
              tenant-id, tenantnaam, slug en owner-invite snapshot.
              Platformgebruikers krijgen geen tenant-id, tenant owners krijgen
              nooit vrije handmatige e-mailtargets.
            </p>
          </div>
        </div>
      </section>
