      ...
      <div className="mx-auto grid w-full max-w-[1300px] gap-6">
        <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold tracking-normal text-slate-950">Releasecategorieen</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Beheer de categorieen waarmee release items worden gegroepeerd per module, audience en impact.
            </p>
          </div>
        ...
        <section className="grid gap-3">
          {categories.map((category) => (
            <div key={category.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <form action={saveCategoryAction} className="grid gap-3 lg:grid-cols-[1fr_1fr_190px_120px_120px]">
              ...
              {category.isActive && (
                <form action={archiveCategoryAction} className="mt-3 flex justify-end">
                  <input type="hidden" name="id" value={category.id} />
