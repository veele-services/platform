...
import type { FieldgridContentAudience, ReleaseHighlightSurface } from "@workspace/db";
      ...
      <div className="mx-auto grid w-full max-w-[1500px] gap-6">
        <header className="border-b border-slate-200 pb-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Bewerk de release, publicatie, audience-scope, module-scope, items en roadmapkoppelingen.
          </p>
        </header>
        ...
        <section className="rounded-lg border border-amber-200 bg-amber-50 p-5 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-950">Highlight / gele balk</h2>
          </div>
          <p className="mt-2 text-sm leading-6 text-slate-700">
            Publiceer een audience-scoped banner. De banner verschijnt alleen wanneer deze release gepubliceerd is, de module actief is en de gebruiker de juiste audience heeft.
          </p>
          <form action={highlightAction} className="mt-4 grid gap-3 lg:grid-cols-[180px_180px_180px_1fr]">
            <input type="hidden" name="releaseId" value={release.id} />
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Surface
              <select name="surface" defaultValue="tenant_backoffice" className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-normal">
                {SURFACES.map((surface) => <option key={surface.key} value={surface.key}>{surface.label}</option>)}
              </select>
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Audience
              <select name="audienceKey" defaultValue="tenant_admin" className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-normal">
                {AUDIENCES.map((audience) => <option key={audience.key} value={audience.key}>{audience.label}</option>)}
              </select>
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Module
              <select name="moduleKey" defaultValue="" className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-normal">
            ...
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Titel
              <input name="title" required defaultValue={`Nieuw: ${release.title}`} className="h-10 rounded-md border border-slate-300 px-3 text-sm font-normal" />
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700 lg:col-span-2">
              Bericht
              <input name="message" required defaultValue={release.summary ?? `Release ${release.version} is beschikbaar.`} className="h-10 rounded-md border border-slate-300 px-3 text-sm font-normal" />
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Prioriteit
              <input name="priority" type="number" defaultValue={50} className="h-10 rounded-md border border-slate-300 px-3 text-sm font-normal" />
            </label>
