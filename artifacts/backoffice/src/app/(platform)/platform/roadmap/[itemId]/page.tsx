        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <label className="mt-4 grid gap-1 text-sm font-medium text-slate-700">
            ...
            <textarea
              ...
              className="min-h-32 rounded-md border border-slate-300 px-3 py-2 text-sm font-normal leading-6"
            />
          </label>
        </section>
      </div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              ...
              <input
                name="statusNote"
                placeholder="Optioneel bij statuswijziging"
                className="h-10 rounded-md border border-slate-300 px-3 text-sm font-normal"
              />
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              ...
              <input
                ...
                type="date"
                defaultValue={item.expectedDelivery?.slice(0, 10) ?? ""}
                className="h-10 rounded-md border border-slate-300 px-3 text-sm font-normal"
              />
            </label>
...
function RoadmapActivity({ item }: { item: RoadmapItemSummary }) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-950">
        Reacties en statusgeschiedenis
      </h2>
      <form action={commentAction} className="mt-4 grid gap-3">
        <input type="hidden" name="id" value={item.id} />
        <textarea
          name="body"
          placeholder="Voeg een interne of tenant-zichtbare reactie toe."
          ...
          <div className="mt-3 grid gap-3">
            {item.statusHistory.map((entry) => (
              ...
              >
                ...
                {entry.note && (
                  <p className="mt-2 leading-6 text-slate-700">{entry.note}</p>
                )}
              </div>
            ))}
            {item.statusHistory.length === 0 && (
              <p className="text-sm text-slate-500">Nog geen statusregels.</p>
            )}
          </div>
        </div>
      </div>
