...
async function breakGlassSensitiveAccessAction(
  formData: FormData,
): Promise<void> {
  "use server";
...
function metadataLabel(metadata: Record<string, unknown> | null): string {
  ...
  const grantType =
    typeof metadata["grantType"] === "string" ? metadata["grantType"] : null;
  const ttlMinutes =
    typeof metadata["ttlMinutes"] === "number" ? metadata["ttlMinutes"] : null;
  const maxTtlMinutes =
    typeof metadata["maxTtlMinutes"] === "number"
      ? metadata["maxTtlMinutes"]
      : null;
  const message =
...
function parseSecurityFilters(
  ...
  return {
    ...
    ) as PlatformSecurityDashboardFilters["eventType"],
    scope: parseFilterValue(
      searchParams.scope,
    ) as PlatformSecurityDashboardFilters["scope"],
    ...
    ) as PlatformSecurityDashboardFilters["severity"],
...
function exportHref(filters: PlatformSecurityDashboardFilters): string {
  ...
  return query
    ? `/api/platform/security/export?${query}`
    : "/api/platform/security/export";
}
  ...
  return (
    <article className="rounded border border-slate-200 bg-white p-4 text-sm shadow-sm">
      <div className="flex flex-wrap items-center gap-2">
        <span
          className={`rounded border px-2 py-1 text-xs font-semibold ${severityClass(event.severity)}`}
        >
      ...
      <h3 className="mt-3 break-words text-base font-semibold tracking-normal text-slate-950">
        {event.action}
      </h3>
  ...
  return (
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className="w-fit rounded bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">
          {events.length}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-xs text-slate-500">
            {formatDate(grant.startsAt)} tot {formatDate(grant.expiresAt)} / TTL{" "}
            {grant.ttlMinutes}m
          </p>
          <p className="mt-1 break-all text-xs text-slate-500">
            Platform user: {grant.platformUserId}
          </p>
        </div>
        {!grant.revokedAt && (
          <form action={revokeSupportGrantAction}>
            ...
            <button
              type="submit"
              className="rounded border border-rose-300 bg-white px-3 py-2 text-xs font-semibold text-rose-800 hover:bg-rose-50"
            >
              Revoke
            </button>
          </form>
    ...
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Reden, scope en expiry zijn verplicht. Max TTL:{" "}
          {FIELDGRID_SUPPORT_BREAK_GLASS_MAX_TTL_MINUTES} minuten.
        </p>
      </div>
      <form
      ...
      >
        ...
        <label className="grid gap-1 text-sm font-medium text-slate-700 sm:col-span-2">
          ...
          <span className="text-xs font-normal text-slate-500">
            Selecteer alleen wat voor deze interventie nodig is.
          </span>
        </label>
        <label className="grid gap-1 text-sm font-medium text-slate-700 sm:col-span-2">
          Reden
...
function sensitiveGrantStatusLabel(
  status: SensitiveAccessGrantRow["status"],
): string {
  if (status === "active") return "Actief";
  if (status === "revoked") return "Ingetrokken";
  return "Verlopen";
...
function SensitiveAccessGrantCard({
  grant,
}: {
  grant: SensitiveAccessGrantRow;
}) {
  return (
    <article className="rounded border border-slate-200 bg-white p-4 text-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded border px-2 py-1 text-xs font-medium ${grant.isActive ? "border-amber-200 bg-amber-50 text-amber-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}
            >
          ...
          <p className="mt-1 text-xs text-slate-500">
            Verloopt: {formatDate(grant.expiresAt)}
          </p>
        </div>
        {grant.isActive && (
          <form
            action={revokeSensitiveAccessAction}
...
function SensitiveAccessRequestCard({
  ...
  return (
    <article className="rounded border border-slate-200 bg-white p-4 text-sm">
      ...
      <p className="mt-1 text-xs text-slate-500">
        Vervalt: {formatDate(request.expiresAt)}
      </p>
      {request.status === "pending" && (
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          <form
            action={approveSensitiveAccessAction}
            className="grid gap-2 rounded border border-emerald-200 bg-emerald-50 p-3"
          >
            <input type="hidden" name="requestId" value={request.id} />
            <SelectAdapter
              name="permission"
              defaultValue="full_read"
              className="h-9 rounded border border-emerald-200 px-2 text-xs"
            >
          ...
          <form
            action={denySensitiveAccessAction}
            className="grid gap-2 rounded border border-rose-200 bg-rose-50 p-3"
          >
            <input type="hidden" name="requestId" value={request.id} />
            <input
              name="reason"
              required
              minLength={12}
              className="h-9 rounded border border-rose-200 px-2 text-xs"
...
function SensitiveAccessPanel({
  dashboard,
}: {
  dashboard: SensitiveAccessDashboard;
}) {
  return (
    <section className="grid gap-5 xl:grid-cols-[420px_minmax(0,1fr)]">
      <div className="grid gap-5">
        <section className="rounded border border-slate-200 bg-white p-5">
          <div className="mb-4">
            <h2 className="text-lg font-semibold tracking-normal text-slate-950">
              Sensitive access aanvragen
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Full-read en export voor gevoelige tenantdata verlopen via
              tijdelijke grants met reden en audit.
            </p>
          </div>
          <form action={requestSensitiveAccessAction} className="grid gap-3">
              ...
              >
                {dashboard.scopeOptions.map((scope) => (
                  <option key={scope.value} value={scope.value}>
                    {scope.label} / level {scope.classification}
                  </option>
                ))}
              </SelectAdapter>
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              ...
              <SelectAdapter
                ...
                className="h-10 rounded border border-slate-300 px-3 text-sm"
              >
                {dashboard.permissionOptions.map((permission) => (
                  <option key={permission} value={permission}>
                    {permission}
                  </option>
                ))}
            ...
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Ticket/reference
              <input
                name="supportTicketReference"
                className="h-10 rounded border border-slate-300 px-3 text-sm"
                placeholder="Ticket of incidentreferentie"
              />
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Reden
        ...
        <section className="rounded border border-rose-200 bg-rose-50 p-5">
          <h2 className="text-lg font-semibold tracking-normal text-rose-950">
            Break-glass sensitive access
          </h2>
          <p className="mt-1 text-sm text-rose-700">
            Alleen voor urgente incidenten. Max TTL: {dashboard.maxTtlMinutes}{" "}
            minuten.
          </p>
          <form
            action={breakGlassSensitiveAccessAction}
            className="mt-4 grid gap-3"
          >
            <SelectAdapter
              name="tenantId"
              required
              className="h-10 rounded border border-rose-200 px-3 text-sm"
            >
              <option value="">Kies tenant</option>
              {dashboard.tenants.map((tenant) => (
                <option key={tenant.id} value={tenant.id}>
            ...
            <textarea
              ...
              minLength={12}
              rows={3}
              className="rounded border border-rose-200 px-3 py-2 text-sm"
              placeholder="Urgente reden"
            />
            <button
              type="submit"
              className="h-10 rounded bg-rose-800 px-4 text-sm font-semibold text-white hover:bg-rose-900"
            >
      ...
      <div className="grid gap-5">
        {dashboard.activeGrants.length > 0 && (
          <section className="rounded border border-amber-200 bg-amber-50 p-5">
            <h2 className="text-lg font-semibold tracking-normal text-amber-950">
              Actieve sensitive grants
            </h2>
            <p className="mt-1 text-sm text-amber-700">
              Deze grants maken full-read of export tijdelijk mogelijk voor de
              genoemde scope.
            </p>
            <div className="mt-4 grid gap-3">
              {dashboard.activeGrants.map((grant) => (
                <SensitiveAccessGrantCard key={grant.id} grant={grant} />
              ))}
            </div>
          </section>
        )}
        <section className="rounded border border-slate-200 bg-white p-5">
          <h2 className="text-lg font-semibold tracking-normal text-slate-950">
            Pending sensitive access
          </h2>
          <div className="mt-4 grid gap-3">
            {dashboard.pendingRequests.map((request) => (
              <SensitiveAccessRequestCard
                key={request.id}
                request={request}
                permissions={dashboard.permissionOptions}
              />
            ))}
            {dashboard.pendingRequests.length === 0 && (
              <p className="platform-empty-state text-sm">
                Geen open sensitive access aanvragen.
              </p>
            )}
          </div>
        </section>
        <section className="rounded border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 grid gap-3">
            ...
            {dashboard.requests.length === 0 && (
              <p className="platform-empty-state text-sm">
                Nog geen sensitive access events.
              </p>
            )}
          </div>
        </section>
  ...
  return (
    <main className="platform-page min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8">
        <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-slate-500">
              Read-only overzicht van support access events, denial events,
              downloads en platformwijzigingen met audit export.
            </p>
        ...
        <form
          method="get"
          className="grid gap-3 rounded border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-4"
        >
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            Tenant
            <SelectAdapter
              name="tenantId"
              defaultValue={dashboard.filters.tenantId ?? ""}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            >
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <SelectAdapter
              name="resource"
              defaultValue={dashboard.filters.resource ?? ""}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            >
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <input
              name="dateFrom"
              type="datetime-local"
              defaultValue={dateInputValue(dashboard.filters.dateFrom)}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            />
          </label>
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <input
              name="dateTo"
              type="datetime-local"
              defaultValue={dateInputValue(dashboard.filters.dateTo)}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            />
          </label>
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <SelectAdapter
              name="severity"
              defaultValue={dashboard.filters.severity}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            >
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <SelectAdapter
              name="supportGrantId"
              defaultValue={dashboard.filters.supportGrantId ?? ""}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            >
              <option value="">Alle grants</option>
              {dashboard.supportGrantOptions.map((grant) => (
                <option key={grant.id} value={grant.id}>
                  {grant.tenantName} / {grant.status} /{" "}
                  {grant.platformUserId.slice(0, 8)}
                </option>
              ))}
            </SelectAdapter>
          </label>
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <SelectAdapter
              name="scope"
              defaultValue={dashboard.filters.scope}
              className="h-10 rounded border border-slate-300 px-3 text-sm"
            >
          ...
          <div className="flex items-end gap-2">
            ...
            <Link
              ...
              className="inline-flex h-10 items-center rounded border border-slate-300 px-3 text-sm font-medium text-slate-700 hover:bg-slate-100"
            >
              Reset
            </Link>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <StatCard
            ...
            value={dashboard.activeSupportGrants.length}
            tone={
              dashboard.activeSupportGrants.length > 0 ? "warning" : "neutral"
            }
          />
          <StatCard
            label="Denials"
            value={dashboard.denialEvents.length}
            tone={dashboard.denialEvents.length > 0 ? "danger" : "neutral"}
          />
          <StatCard
            label="Waarschuwingen"
            value={dashboard.severityCounts.warning}
            tone={dashboard.severityCounts.warning > 0 ? "warning" : "neutral"}
          />
          <StatCard
            label="Kritiek"
            value={dashboard.severityCounts.critical}
            tone={dashboard.severityCounts.critical > 0 ? "danger" : "neutral"}
          />
        </div>
        ...
        <section className="rounded border border-slate-200 bg-white p-5">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Direct-ID denial, module denial, storage denial, tenant mismatch en
            platform access denial worden apart geteld.
          </p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {(
              Object.entries(denialLabels) as [
                PlatformSecurityDenialType,
                string,
              ][]
            ).map(([key, label]) => (
              <div
        ...
        <div className="grid gap-5 xl:grid-cols-[420px_minmax(0,1fr)]">
          <SupportGrantForm
            tenants={dashboard.tenantOptions}
          ...
          <section className="rounded border border-slate-200 bg-white p-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Support grant zonder reden, scope of geldige expiry faalt.
              Verlopen grants zijn niet actief.
            </p>
            <div className="mt-4 grid gap-3">
              {dashboard.activeSupportGrants.map((grant) => (
                <SupportGrantCard key={grant.id} grant={grant} />
              ))}
