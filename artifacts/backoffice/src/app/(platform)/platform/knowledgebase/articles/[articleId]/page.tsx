...
export async function generateMetadata({ params }: Props) {
  ...
  const article = await getKnowledgebaseArticleForEdit(articleId);
  return { title: article?.title ?? "Knowledgebase-artikel" };
}

export default async function EditKnowledgebaseArticlePage({ params }: Props) {
  const { articleId } = await params;
  const [article, options] = await Promise.all([
    getKnowledgebaseArticleForEdit(articleId),
    listKnowledgebaseEditorOptions(articleId),
  ]);
        ...
        <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Beheer inhoud, zichtbaarheid, media en gerelateerde artikelen.
            </p>
          </div>
        </header>
