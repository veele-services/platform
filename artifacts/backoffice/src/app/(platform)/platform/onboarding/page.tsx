...
function statusClass(status: PlatformOnboardingPreflightStatus | PlatformProvisioningStepRow["status"]): string {
  if (status === "ready" || status === "completed") return "border-emerald-200 bg-emerald-50 text-emerald-800";
  if (status === "warning" || status === "active") return "border-amber-200 bg-amber-50 text-amber-800";
  if (status === "blocked" || status === "failed" || status === "rolled_back") return "border-rose-200 bg-rose-50 text-rose-800";
  return "border-slate-200 bg-slate-50 text-slate-600";
}

function StatusIcon({ status }: { status: PlatformOnboardingPreflightStatus | PlatformProvisioningStepRow["status"] }) {
  if (status === "ready" || status === "completed") return <CheckCircle2 aria-hidden="true" className="size-4" />;
  if (status === "blocked" || status === "failed" || status === "rolled_back") return <AlertTriangle aria-hidden="true" className="size-4" />;
...
function Badge({
  ...
  children: ReactNode;
  status: PlatformOnboardingPreflightStatus | PlatformProvisioningStepRow["status"];
}) {
  return (
    <span className={`inline-flex w-fit items-center gap-1 rounded border px-2.5 py-1 text-xs font-semibold ${statusClass(status)}`}>
      ...
      <ol className="flex min-w-max gap-2">
        {WIZARD_STEPS.map((step, index) => (
          <li key={step.id} className="flex items-center gap-2">
            ...
            <span className="max-w-36 text-sm font-medium text-slate-700">{step.label}</span>
          </li>
        ))}
      </ol>
    </nav>
...
function PreflightPanel({ preflight }: { preflight: PlatformOnboardingPreflight | null }) {
  return (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Duplicate slug/domain, owner invite, Fieldgrid subdomain en tenant first-run readiness worden server-side beoordeeld.
          </p>
        </div>
        <Badge status={preflight?.status ?? "warning"}>{preflight?.status ?? "concept"}</Badge>
      </div>
      {preflight ? (
        <div className="grid gap-2 md:grid-cols-2">
          {preflight.checks.map((check) => (
            <div key={check.id} className={`rounded border p-3 ${statusClass(check.status)}`}>
        ...
        <p className="text-sm text-slate-500">Sla een concept op om preflightstatus direct zichtbaar en hervatbaar te maken.</p>
      )}
    </section>
  );
}

function OnboardingWizard({ workspace }: { workspace: Awaited<ReturnType<typeof getPlatformOnboardingWorkspace>> }) {
  ...
  return (
    <section className="grid gap-5 rounded border border-slate-200 bg-white p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-xl font-semibold tracking-normal text-slate-950">Onboarding 2.0 wizard</h2>
          <p className="mt-1 text-sm text-slate-500">
            Nieuwe tenants worden zonder SQL aangemaakt met save/resume, review, provisioning run, veilige retry en rollback.
          </p>
        </div>
        {draft ? (
          <Badge status={preflight?.status ?? "warning"}>Concept {draft.currentStep}</Badge>
        ) : (
      ...
      <form action={createPlatformTenant} className="grid gap-6">
        ...
        <FieldGroup title="Tenantgegevens" helper="Naam en slug bepalen tenantidentiteit en hostcontext.">
          <div className="grid gap-3 md:grid-cols-[1fr_0.7fr]">
            <TextInput label="Tenantnaam" name="name" required placeholder="Demo X" defaultValue={fieldValue(draft?.name)} />
            <TextInput label="Slug" name="slug" placeholder="demo-x" defaultValue={fieldValue(draft?.slug)} />
          </div>
        </FieldGroup>

        <FieldGroup title="Plan en Fieldgrid subdomain" helper="Voor demo-x is demo-x.fieldgrid.nl direct bereikbaar na provisioning.">
          <div className="grid gap-3 md:grid-cols-3">
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Plan
              <select name="planKey" defaultValue={selectedPlan} className="min-h-11 rounded border border-slate-300 bg-white px-3 py-2 text-sm">
                {plans.map((plan) => (
            ...
            <TextInput label="Fieldgrid subdomain" name="fieldgridSubdomain" placeholder="demo-x" defaultValue={fieldValue(fieldgridSubdomain)} />
            <TextInput label="Primair domein override" name="domain" placeholder="demo-x.fieldgrid.nl" defaultValue={fieldValue(draft?.primaryDomain)} />
          </div>
        </FieldGroup>

        <FieldGroup title="Modules" helper="Kies modules expliciet, of laat leeg om plandefaults te gebruiken.">
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {catalog.modules.map((module) => (
              <label key={module.key} className="flex min-h-20 gap-3 rounded border border-slate-200 px-3 py-2 text-sm">
                <input
                  type="checkbox"
                  name="moduleKeys"
                  value={module.key}
        ...
        <FieldGroup title="Sectoren en regio's" helper="Deze stap zaait sectorbeleid, defaultsector en tenantregio's.">
          <div className="grid gap-3 md:grid-cols-2">
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Sectorbeleid
              <select name="sectorMode" defaultValue={selectedSectorMode} className="min-h-11 rounded border border-slate-300 bg-white px-3 py-2 text-sm">
            ...
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Defaultsector
              <select name="defaultSectorId" defaultValue={selectedDefaultSectorId} className="min-h-11 rounded border border-slate-300 bg-white px-3 py-2 text-sm">
        ...
        <FieldGroup title="Branding en owner invite" helper="Owner invite en tenant first-run readiness worden aan de provisioning run gekoppeld.">
          <div className="grid gap-3 md:grid-cols-[1fr_0.5fr_0.5fr]">
            <TextInput label="Branding displaynaam" name="brandingDisplayName" placeholder="Demo X" defaultValue={fieldValue(draft?.branding.displayName)} />
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Primaire kleur
              <input name="primaryColor" type="color" defaultValue={draft?.branding.primaryColor ?? DEFAULT_PRIMARY_COLOR} className="h-11 rounded border border-slate-300 bg-white px-2 py-1" />
            </label>
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              Accentkleur
              <input name="accentColor" type="color" defaultValue={draft?.branding.accentColor ?? DEFAULT_ACCENT_COLOR} className="h-11 rounded border border-slate-300 bg-white px-2 py-1" />
            </label>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <TextInput label="Owner e-mail" name="ownerEmail" type="email" required placeholder="owner@example.nl" defaultValue={fieldValue(draft?.ownerEmail)} />
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              E-mailhandtekening
              <textarea
                name="emailSignature"
        ...
        <FieldGroup title="Review" helper="Reviewnotitie blijft in de provisioningmetadata staan voor save/resume, retry en rollbackcontext.">
          <textarea
            name="reviewNotes"
            rows={4}
            className="rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200"
...
function RunSteps({ steps }: { steps: PlatformProvisioningStepRow[] }) {
  return (
    <ol className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
      {steps.map((step) => (
        <li key={step.id} className={`rounded border p-3 ${statusClass(step.status)}`}>
          <div className="flex items-center gap-2">
    ...
    <article className="grid gap-4 rounded border border-slate-200 bg-white p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 break-all text-sm text-slate-500">{run.slug} / {run.primaryDomain ?? "geen domein"}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Badge status={run.status === "failed" ? "failed" : run.status === "rolled_back" ? "rolled_back" : run.status === "draft" ? "active" : "completed"}>
            {run.status}
          </Badge>
          <Badge status={run.ownerInviteStatus === "sent" || run.ownerInviteStatus === "accepted" ? "completed" : "active"}>
            {run.readinessLabel}
          </Badge>
        </div>
      </div>
      ...
      <div className="grid gap-3 text-sm text-slate-600 md:grid-cols-3">
        <p><span className="font-semibold text-slate-800">Plan:</span> {run.planKey}</p>
        <p><span className="font-semibold text-slate-800">Owner:</span> {run.ownerEmail ?? "-"}</p>
        <p><span className="font-semibold text-slate-800">Gestart:</span> {formatDate(run.startedAt)}</p>
        <p><span className="font-semibold text-slate-800">Modules:</span> {run.moduleKeys.length}</p>
        <p><span className="font-semibold text-slate-800">Sectoren:</span> {run.sectorIds.length}</p>
        <p><span className="font-semibold text-slate-800">Regio's:</span> {run.regionNames.length}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {run.canRetry && (
          <form action={retryPlatformTenantProvisioning}>
            ...
            <button type="submit" className="inline-flex min-h-10 items-center gap-2 rounded border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
        ...
        {run.canRollback && (
          <form action={rollbackPlatformTenantProvisioning}>
            ...
            <button type="submit" className="inline-flex min-h-10 items-center gap-2 rounded border border-rose-300 px-3 py-2 text-sm font-semibold text-rose-700 transition hover:bg-rose-50">
  ...
  return (
    <section id="provisioning-runs" className="grid gap-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold tracking-normal text-slate-950">Provisioning runs</h2>
          <p className="mt-1 text-sm text-slate-500">Runstatus, stappen, owner invite, tenant first-run readiness, retry en rollback.</p>
        </div>
      ...
      <div className="grid gap-3">
        ...
        {runs.length === 0 && (
          <div className="rounded border border-slate-200 bg-white p-5 text-sm text-slate-500">
            Nog geen provisioning runs. Sla een concept op of provision de eerste tenant.
          </div>
        )}
      </div>
    </section>
