...
function statusChipClass(status: string): string {
  ...
  if (status === "waiting_customer" || status === "waiting_internal")
...
function LinkSelect({
  ...
  return (
    <label className="grid gap-1 text-sm font-medium text-slate-700">
      ...
      <SelectAdapter
        name={name}
        defaultValue={defaultValue ?? "__none"}
    ...
    <section className="rounded border border-slate-200 bg-white p-5">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Maak support-, incident-, onboarding-, billing-, domein- of
          securitytickets met interne opvolging.
        </p>
      </div>
      ...
      <form action={createTicketFormAction} className="grid gap-3">
        ...
        <label className="grid gap-1 text-sm font-medium text-slate-700">
          ...
          <textarea
            ...
            placeholder="Context, impact, vervolgstap of klantvraag"
          />
        </label>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <LinkSelect
            name="subscriptionId"
            label="Abonnement"
            options={dashboard.subscriptions}
          />
          <LinkSelect
            name="domainId"
            label="Domein"
            options={dashboard.domains}
          />
          <LinkSelect
            name="supportGrantId"
            label="Support grant"
            options={dashboard.supportGrants}
          />
          <LinkSelect
            name="auditLogId"
            label="Audit event"
            options={dashboard.auditEvents}
          />
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            Smoke run ID
            <input
              name="smokeRunId"
              className="h-10 rounded border border-slate-300 px-3 text-sm"
              placeholder="dashboard-snapshot / run-id"
            />
          </label>
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            Toegewezen aan
            <SelectAdapter
              name="assigneePlatformUserId"
              defaultValue="__none"
              className="h-10 rounded border border-slate-300 bg-white px-3 text-sm"
            >
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            className="h-10 rounded bg-slate-950 px-4 text-sm font-semibold text-white transition hover:bg-slate-800"
          >
            Ticket aanmaken
          </button>
        </div>
...
function TicketCard({
  ...
  return (
    <article className="rounded border border-slate-200 bg-white p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded border px-2 py-1 text-xs font-medium ${statusChipClass(ticket.status)}`}
            >
              {STATUS_LABELS[ticket.status] ?? ticket.status}
            </span>
            <span
              className={`rounded border px-2 py-1 text-xs font-medium ${priorityChipClass(ticket.priority, ticket.isOverdue)}`}
            >
              {ticket.isOverdue
                ? "SLA verlopen"
          ...
          <Link
            ...
            className="mt-2 block break-words text-lg font-semibold text-slate-950 underline-offset-2 hover:underline"
          >
            {ticket.title}
          </Link>
          <p className="mt-1 text-sm text-slate-500">
            Laatste activiteit {formatDate(ticket.lastActivityAt)} - aangemaakt{" "}
          ...
          <div className="rounded bg-slate-50 px-3 py-2">
            ...
            {ticket.tenantId ? (
              <Link
                ...
                className="mt-1 block truncate font-medium text-slate-950 underline-offset-2 hover:underline"
              >
          ...
          <div className="rounded bg-slate-50 px-3 py-2">
            ...
            <p className="mt-1 truncate font-medium text-slate-950">
              {[
                ...
                ticket.supportGrantLabel,
                ticket.smokeRunId,
                ticket.auditLogLabel,
              ]
                .filter(Boolean)
                .join(" / ") || "-"}
            </p>
      ...
      <form
        action={updateTicketFormAction}
        className="mt-4 grid gap-2 rounded border border-slate-200 bg-slate-50 p-3 md:grid-cols-[170px_150px_190px_minmax(0,1fr)_auto] md:items-end"
      >
        ...
        <label className="grid gap-1 text-sm font-medium text-slate-700">
          ...
          <input
            name="slaDueAt"
            type="datetime-local"
            defaultValue={dateTimeLocalValue(ticket.slaDueAt)}
            className="h-10 rounded border border-slate-300 bg-white px-3 text-sm"
          />
        </label>
        <label className="grid gap-1 text-sm font-medium text-slate-700">
          ...
          <SelectAdapter
            name="assigneePlatformUserId"
            defaultValue={ticket.assigneePlatformUserId ?? "__none"}
            className="h-10 rounded border border-slate-300 bg-white px-3 text-sm"
          >
            <option value="__none">Niet toegewezen</option>
            {dashboard.platformUsers.map((user) => (
        ...
        <button
          type="submit"
          className="h-10 rounded border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-800 transition hover:bg-slate-100"
        >
          Opslaan
        </button>
      </form>
    ...
    <main className="platform-page mx-auto flex w-full max-w-7xl flex-col gap-5 p-4 md:p-6">
      <div className="flex flex-col gap-2 border-b border-slate-200 pb-4">
        ...
        <div>
          <p className="text-sm font-medium text-slate-500">
            Fieldgrid operations
          </p>
          <h1 className="text-3xl font-semibold tracking-normal text-slate-950">
            Platformtickets
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Supportvragen, incidenten, onboarding, billing, domeinen en
            security-opvolging voor platformbeheer.
          </p>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat
          ...
          icon={AlertTriangle}
          tone={dashboard.stats.overdue > 0 ? "danger" : "good"}
        />
        <Stat
          label="Hoog/urgent"
          value={dashboard.stats.highPriority}
          icon={ShieldAlert}
          tone={dashboard.stats.highPriority > 0 ? "danger" : "default"}
        />
        <Stat
          label="Opgelost"
          value={dashboard.stats.resolved}
      ...
      <section className="grid gap-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Maximaal 250 recent actieve tickets, inclusief platform-only
              tickets en tenantkoppelingen.
            </p>
          </div>
