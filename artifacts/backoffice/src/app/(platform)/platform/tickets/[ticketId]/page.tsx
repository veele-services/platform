...
function chipClass(
  ...
  if (value === "waiting_customer" || value === "waiting_internal")
  ...
  return (
    <div className="rounded border border-slate-200 bg-white px-4 py-3">
      ...
      {href && value ? (
        <Link
          href={href}
          className="mt-1 block break-words text-sm font-semibold text-slate-950 underline-offset-2 hover:underline"
        >
          {value}
        </Link>
      ) : (
    ...
    <section className="rounded border border-slate-200 bg-white p-5">
      ...
      <div className="grid gap-3">
        <LinkedItem
          ...
          value={
            ticket.tenantName ??
            ticket.tenantSlug ??
            (ticket.tenantId ? ticket.tenantId : "Platform-only")
          }
        ...
        <LinkedItem
          label="Audit event"
          value={ticket.auditLogLabel ?? ticket.auditLogId}
        />
      </div>
      ...
      >
        ...
        <input
          type="hidden"
          name="slaDueAt"
          value={dateTimeLocalValue(ticket.slaDueAt)}
        />
        <input
          type="hidden"
          name="assigneePlatformUserId"
          value={ticket.assigneePlatformUserId ?? "__none"}
        />
        <LinkSelect
          name="subscriptionId"
          label="Abonnement"
          options={ticket.linkOptions.subscriptions}
          defaultValue={ticket.subscriptionId}
        />
        <LinkSelect
          name="domainId"
          label="Domein"
          options={ticket.linkOptions.domains}
          defaultValue={ticket.domainId}
        />
        <LinkSelect
          name="supportGrantId"
          label="Support grant"
          options={ticket.linkOptions.supportGrants}
          defaultValue={ticket.supportGrantId}
        />
        <LinkSelect
          name="auditLogId"
          label="Audit event"
          options={ticket.linkOptions.auditEvents}
          defaultValue={ticket.auditLogId}
        />
        <button
          type="submit"
          ...
          >
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              ...
              <p className="text-xs text-slate-500">
                {formatDate(note.createdAt)} - {note.visibility}
              </p>
            </div>
    ...
    <main className="platform-page mx-auto flex w-full max-w-7xl flex-col gap-5 p-4 md:p-6">
      <div className="flex flex-col gap-3 border-b border-slate-200 pb-4">
        <Link
          ...
          className="inline-flex w-fit items-center gap-2 text-sm text-slate-500 underline-offset-2 hover:underline"
        >
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <span
                className={`rounded border px-2 py-1 text-xs font-medium ${chipClass("status", ticket.status)}`}
              >
                {STATUS_LABELS[ticket.status] ?? ticket.status}
              </span>
              <span
                className={`rounded border px-2 py-1 text-xs font-medium ${chipClass("priority", ticket.priority, ticket.isOverdue)}`}
              >
                {ticket.isOverdue
                  ? "SLA verlopen"
            ...
            <p className="mt-2 text-sm text-slate-500">
              Laatste activiteit {formatDate(ticket.lastActivityAt)} - SLA{" "}
          ...
          <div className="rounded border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 lg:min-w-[280px]">
            <div className="flex items-center gap-2">
              ...
              <p className="font-semibold text-slate-950">
                {ticket.noteCount} interne notitie(s)
              </p>
            </div>
