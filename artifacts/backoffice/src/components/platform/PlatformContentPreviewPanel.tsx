...
import type {
  PlatformContentPreviewModel,
  PlatformPreviewResource,
} from "@/lib/platform-content-preview";
...
function previewPermissionSummary(values: string[]): string {
  ...
  const resources = [
    ...new Set(values.map((value) => value.split(":")[0]).filter(Boolean)),
  ];
  const preview = resources
    .slice(0, 8)
    .map((resource) => resource.replace(/_/g, " "))
    .join(", ");
...
export function PlatformContentPreviewPanel({
  ...
  const visibleItems = snapshot.items
    ...
    .slice(0, 5);
  const hiddenItems = snapshot.items
    .filter((item) => !item.visible)
    .slice(0, 5);
  const moduleNameByKey = new Map(
    options.modules.map((module) => [module.key, module.name]),
  );
  const moduleLabel = (key: string) =>
    moduleNameByKey.get(key) ?? key.replace(/_/g, " ");
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-950">
              Preview als audience / tenant / rol
            </h2>
          </div>
          <p className="mt-1 text-sm leading-6 text-slate-600">
            Controleer met dezelfde runtime visibility-code wat zichtbaar is
            voor {resourceLabel(resource).toLowerCase()}.
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <Badge variant="outline" className="bg-white">
              {SURFACE_LABELS[snapshot.surface] ??
                humanizePreviewText(snapshot.surface)}
            </Badge>
            <Badge
              variant="outline"
              className="border-emerald-200 bg-emerald-50 text-emerald-700"
            >
              {snapshot.visibleCount} zichtbaar
            </Badge>
            <Badge
              variant="outline"
              className="border-amber-200 bg-amber-50 text-amber-800"
            >
              {snapshot.hiddenCount} verborgen
            </Badge>
          </div>
        </div>
        ...
        <form className="grid gap-3 rounded-lg border border-cyan-100 bg-white p-3">
          ...
          <label className="grid gap-1 text-sm font-medium text-slate-700">
            ...
            <span className="text-xs font-normal text-slate-500">
              Leeg laten gebruikt de actieve modules van de gekozen tenant.
            </span>
          </label>
          ...
          <dl className="mt-3 grid gap-2 text-sm">
            <div>
              ...
              <dd className="mt-1 break-words text-slate-700">
                {joinPreviewLabels(snapshot.runtimeAudiences, audienceLabel)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">
              ...
              <dd className="mt-1 break-words text-slate-700">
                {joinPreviewLabels(snapshot.activeModuleKeys, moduleLabel)}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-semibold uppercase tracking-[0.08em] text-slate-500">
              ...
              <dd className="mt-1 break-words text-slate-700">
                {previewPermissionSummary(snapshot.permissionKeys)}
              </dd>
            </div>
          </dl>
        ...
        {items.map((item) => (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <span
                className={`rounded-full border px-2 py-1 text-xs font-semibold ${statusClass(item.visible)}`}
              >
                {item.visible ? "zichtbaar" : "verborgen"}
              </span>
            </div>
            <div className="mt-2 flex flex-wrap gap-1">
              {item.audienceKeys.map((audience) => (
            ...
            {(item.visible ? item.matched : item.reasons).length > 0 && (
              <ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-5 text-slate-600">
                {(item.visible ? item.matched : item.reasons)
                  .slice(0, 4)
                  .map((reason) => (
