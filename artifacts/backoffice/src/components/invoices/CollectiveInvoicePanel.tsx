...
export function CollectiveInvoicePanel({
  candidates,
  batches,
  canWrite,
}: Props) {
  ...
  const selectableCandidates = candidates.filter((candidate) => {
    if (selectedCustomerId && candidate.customerId !== selectedCustomerId)
      return false;
    if (objectId && candidate.objectId !== objectId) return false;
    if (
      periodStart &&
      candidate.scheduledDate &&
      candidate.scheduledDate < periodStart
    )
      return false;
    if (
      periodEnd &&
      candidate.scheduledDate &&
      candidate.scheduledDate > periodEnd
    )
      return false;
    return true;
  });
  ...
  const objectOptions = useMemo(() => {
    ...
    for (const candidate of candidates) {
      if (candidate.objectId && candidate.objectName)
        map.set(candidate.objectId, candidate.objectName);
    }
    return [...map.entries()].map(([id, name]) => ({ id, name }));
  }, [candidates]);

  const subtotal = selected.reduce(
    (sum, invoice) => sum + cents(invoice.amount),
    0,
  );
  const vat = selected.reduce(
    (sum, invoice) => sum + cents(invoice.vatAmount),
    0,
  );
  const invoiceTotal = selected.reduce(
    (sum, invoice) => sum + cents(invoice.totalAmount),
    0,
  );
  const discount = Math.max(
    0,
    Math.round((Number.parseFloat(discountEuros || "0") || 0) * 100),
  );
  const surcharge = Math.max(
    0,
    Math.round((Number.parseFloat(surchargeEuros || "0") || 0) * 100),
  );
  const total = invoiceTotal - discount + surcharge;
  const customerMismatch =
    selected.length > 1 &&
    selected.some((invoice) => invoice.customerId !== selectedCustomerId);
  const canSubmit =
    canWrite &&
    selected.length >= 2 &&
    !customerMismatch &&
    ...
    <TenantWorkbenchPanel
      ...
      actions={
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent
            side="right"
            className="flex w-full flex-col overflow-y-auto sm:max-w-3xl"
          >
            <SheetHeader>
              <SheetTitle>Verzamelfactuur wizard</SheetTitle>
              <SheetDescription>
                Selecteer minimaal twee verzonden facturen van dezelfde klant.
                Open of betaalde batches blokkeren dubbele facturatie.
              </SheetDescription>
            </SheetHeader>
              ...
              <div className="grid gap-3 md:grid-cols-5">
                ...
                <label className="grid gap-1 text-sm font-medium">
                  ...
                  <input
                    ...
                    min="0"
                    value={discountEuros}
                    onChange={(event) => setDiscountEuros(event.target.value)}
                    className="h-10 rounded-md border border-input bg-background px-3 text-sm"
                  />
                </label>
                <label className="grid gap-1 text-sm font-medium">
                  ...
                  <input
                    ...
                    onChange={(event) => setSurchargeEuros(event.target.value)}
                    className="h-10 rounded-md border border-input bg-background px-3 text-sm"
                  />
                </label>
              </div>
                ...
                <div className="max-h-[480px] overflow-auto rounded-lg border border-border">
                  {selectableCandidates.length === 0 ? (
                    <p className="p-4 text-sm text-muted-foreground">
                      Geen bundelbare open facturen gevonden.
                    </p>
                  ) : (
                          ...
                          >
                            ...
                            <td className="px-3 py-2">
                              ...
                              <div className="text-xs text-muted-foreground">
                                ...
                                {invoice.scheduledDate ?? "geen datum"}
                              </div>
                            </td>
                            <td className="px-3 py-2">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {invoice.objectName ?? "Geen object"}
                              </div>
                            </td>
                ...
                <div className="space-y-3">
                  <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm">
                    ...
                    <div className="mt-2 flex justify-between">
                      <span>Subtotaal</span>
                      <span>{formatEurCents(subtotal)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Btw</span>
                      <span>{formatEurCents(vat)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Korting</span>
                      <span>- {formatEurCents(discount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Toeslag</span>
                      <span>{formatEurCents(surcharge)}</span>
                    </div>
                    <div className="mt-2 flex justify-between border-t border-border pt-2 font-semibold text-foreground">
                      <span>Te betalen</span>
                      <span>{formatEurCents(total)}</span>
                    </div>
                  </div>
                  <textarea
                    value={notes}
                  ...
                  {customerMismatch && (
                    <p className="text-xs text-red-600">
                      Selecteer alleen facturen van dezelfde klant.
                    </p>
                  )}
                </div>
              </div>
    ...
    >
      <div className="grid gap-3 p-4 md:grid-cols-3">
        <Metric
          ...
          helper="verzonden facturen"
        />
        <Metric
          label="Open batches"
          value={batches.filter((batch) => batch.status === "open").length}
          helper="wachten op betaling"
        />
        <Metric
          label="Geselecteerd"
          value={selected.length}
          helper={formatEurCents(total)}
        />
      </div>
            ...
            {batches.slice(0, 6).map((batch) => (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  {batch.invoiceCount} facturen -{" "}
                  {formatEurCents(batch.amountCents)}
                  {batch.objectName ? ` - ${batch.objectName}` : ""}
                </p>
                <div className="mt-3 flex flex-wrap gap-3">
                  <a
                    href={backofficePath(
                      `/backoffice-api/invoices/batches/${batch.id}/pdf`,
                    )}
                    target="_blank"
                    rel="noreferrer"
