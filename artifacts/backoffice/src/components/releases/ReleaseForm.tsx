...
export function ReleaseForm({ release, options }: ReleaseFormProps) {
  ...
  const [impactLevel, setImpactLevel] = useState<ReleaseImpactLevel>(
    release?.impactLevel ?? "medium",
  );
  ...
  const [roadmapItemIds, setRoadmapItemIds] = useState<string[]>(
    release?.roadmapItems.map((item) => item.id) ?? [],
  );
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <div className="grid gap-4 md:grid-cols-[180px_minmax(0,1fr)_260px]">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Planning en release highlights"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Splits de release in concrete wijzigingen per module of
                categorie.
              </p>
            </div>
            <Button
              ...
              >
                <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_180px_130px_auto]">
                  ...
                  <SelectAdapter
                    ...
                    onChange={(event) =>
                      updateItem(index, {
                        impactLevel: event.target.value as ReleaseImpactLevel,
                      })
                    }
                    className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm"
                  >
        ...
        >
          <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Upload screenshots, video's of PDF-bijlagen en voeg ze daarna
                inline toe in de release notes.
              </p>
            </div>
            <Badge variant="outline">
              {release?.media.length ?? 0} bestanden
            </Badge>
          </div>
          <form
            onSubmit={uploadMedia}
            className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_220px_220px_110px_auto]"
          >
            <Input
              ref={fileInputRef}
              name="file"
            ...
            <Button
              type="submit"
              variant="outline"
              disabled={isUploading || !release?.id}
              className="gap-2"
            >
              {isUploading ? (
          ...
          {!release?.id && (
            <p className="mt-2 text-xs text-slate-500">
              Maak de release eerst aan voordat media veilig gekoppeld kan
              worden.
            </p>
          )}
            ...
            {(release?.media ?? []).map((item) => (
              ...
              >
                {item.mediaType === "image" ? (
                  <img
                    src={backofficePath(`/platform/releases/media/${item.id}`)}
                    alt={item.altText ?? item.caption ?? "Release media"}
      ...
      <aside className="grid gap-5 self-start">
          ...
          <div className="mt-4 grid gap-3">
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              ...
              <SelectAdapter
                value={status}
                onChange={(event) =>
                  setStatus(event.target.value as ReleaseStatus)
                }
            ...
            <label className="grid gap-1 text-sm font-medium text-slate-700">
              ...
              <SelectAdapter
                value={impactLevel}
                onChange={(event) =>
                  setImpactLevel(event.target.value as ReleaseImpactLevel)
                }
                className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-normal"
              >
                <option value="low">Low</option>
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setModuleKeys((current) =>
                          toggleValue(current, module.key, Boolean(checked)),
                        )
                      }
                    />
                    {module.name}
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Gebruik Ctrl/Cmd om meerdere roadmapitems te selecteren.
          </p>
        </section>
