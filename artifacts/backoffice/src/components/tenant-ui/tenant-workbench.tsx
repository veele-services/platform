...
export interface TenantCommandBarProps extends Omit<
    ...
    >
      {items.map((item, index) => {
        ...
        const content = (
          <div
            className={cn(
              "h-full rounded-lg border px-3 py-2.5 shadow-card",
              toneClass[tone],
            )}
          >
...
export function TenantWorkbenchLayout({
  ...
  return (
    <div
      className={cn(
        aside
          ? "grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_360px]"
          : "grid gap-3",
        className,
      )}
      {...props}
...
export interface TenantWorkbenchPanelProps extends Omit<
