...
const CHANNELS: Array<{
...
}> = [
  {
    ...
    label: "E-mail",
    Icon: Mail,
    hint: "HTML-mail via de ingestelde e-mailprovider",
  },
  {
    key: "push",
    label: "Push",
    Icon: Smartphone,
    hint: "Pushbericht + inboxmelding",
  },
...
const SHORTCODE_LABELS = new Map<string, string>(
  SHORTCODE_HELP.map(([code, label]) => [code, label]),
);
...
function groupEvents(events: NotificationEventSettingRow[]) {
  return events.reduce<Record<string, NotificationEventSettingRow[]>>(
    (acc, event) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-5 lg:grid-cols-[22rem_1fr]">
          ...
          <div className="mb-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p
                ...
                style={{ color: "#64748B" }}
              >
                Dit zijn de acties die e-mail, push of inboxmeldingen kunnen
                activeren.
              </p>
            </div>
          </div>
                  ...
                  {groupEvents.map((event) => (
                    ...
                    >
                      ...
                      <span className="mt-2 flex flex-wrap gap-1.5">
                        <ChannelBadge
                          active={event.emailEnabled}
                          label="Mail"
                        />
                        <ChannelBadge active={event.pushEnabled} label="Push" />
                        <ChannelBadge
                          active={event.inAppEnabled}
                          label="Inbox"
                        />
                      </span>
                    </button>
        ...
        {selectedEvent && (
          <EventTemplateEditor event={selectedEvent} canWrite={canWrite} />
        )}
      </section>
  ...
  return (
    <section className="grid gap-3 md:grid-cols-4">
      <MetricCard
        title="Triggers"
        value={events.length}
        text="Platformacties met template"
        Icon={Bell}
      />
      <MetricCard
        title="E-mail actief"
        value={emailCount}
        text="HTML-templates ingeschakeld"
        Icon={Mail}
      />
      <MetricCard
        title="Push actief"
        value={pushCount}
        text="Pushverzending voorbereid"
        Icon={Smartphone}
      />
      <MetricCard
        title="Inbox actief"
        value={inAppCount}
        text="Portaalmeldingen"
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-3">
          ...
          <Field label="E-mail body">
            <textarea
              className="veele-input min-h-[220px] w-full resize-y"
              value={emailBody}
              disabled={!canWrite}
        ...
        <div className="space-y-3">
          ...
          <Field label="Push tekst">
            <textarea
              className="veele-input min-h-[120px] w-full resize-y"
              value={pushBody}
              disabled={!canWrite}
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {event.shortcodes.map((code) => (
                ...
                >
                  ...
                  <code className="ml-1 text-[11px] text-slate-500">
                    {shortcodeToken(code)}
                  </code>
                </span>
              ))}
...
function ManualNotificationPanel({
  ...
  const [isPending, startTransition] = useTransition();
  const [notice, setNotice] = useState<Notice>(null);
  const [audience, setAudience] = useState<"personnel" | "customer" | "both">(
    "personnel",
  );
  const [targetMode, setTargetMode] = useState<"all" | "sector" | "individual">(
    "all",
  );
    ...
    <section className="veele-card space-y-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p
            ...
            style={{ color: "#64748B" }}
          >
            Verstuur een gerichte melding naar personeel, klanten, sectoren of
            individuele ontvangers. Push wordt klaargezet voor verzending en
            tegelijk als inboxmelding bewaard.
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {(audience === "personnel" || audience === "both") && (
            <CheckboxGrid
              ...
              onToggle={(id) =>
                setPersonnelIds((current) => toggle(current, id))
              }
              disabled={!canWrite}
            />
          )}
          {(audience === "customer" || audience === "both") && (
            <CheckboxGrid
              ...
              onToggle={(id) =>
                setCustomerIds((current) => toggle(current, id))
              }
              disabled={!canWrite}
            />
          )}
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_18rem]">
        <div className="space-y-3">
          ...
          <Field label="Bericht">
            <textarea
              className="veele-input min-h-[180px] w-full resize-y"
              value={body}
              disabled={!canWrite}
        ...
        <div className="space-y-3">
          ...
          <div
            className="rounded-xl bg-slate-50 p-3 text-xs leading-relaxed"
            style={{ color: "#64748B" }}
          >
            Gebruik bijvoorbeeld <code>{"{{recipient.name}}"}</code> voor een
            persoonlijke aanhef. Voor handmatige berichten zijn ook{" "}
            <code>{"{{customer.name}}"}</code> en{" "}
            <code>{"{{personnel.first_name}}"}</code> beschikbaar.
          </div>
        </div>
      </div>
...
function EmailStylePanel({
  ...
  const [signature, setSignature] = useState(
    settings?.emailTemplateSignature ?? "",
  );
  ...
  return (
    <section className="veele-card space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p
            ...
            style={{ color: "#64748B" }}
          >
            Deze stijl wordt gebruikt door alle nieuwe professionele HTML
            e-mails.
          </p>
        </div>
      </div>
      ...
      <Field label="Handtekening">
        <textarea
          className="veele-input min-h-[90px] w-full resize-y"
          value={signature}
          disabled={!canWrite}
      ...
      <Field label="Voettekst">
        <textarea
          className="veele-input min-h-[100px] w-full resize-y"
          value={footerText}
          disabled={!canWrite}
  ...
  return (
    <section className="veele-card">
      ...
      <p className="mt-1 text-xs leading-relaxed" style={{ color: "#64748B" }}>
        Shortcodes worden bij automatische events vervangen door de context van
        de opdracht, klant, medewerker, offerte, factuur of nieuwsbericht. Per
        event ziet u de toegestane shortcodes in de editor.
      </p>
      <div className="mt-4 grid gap-2 md:grid-cols-2">
        {SHORTCODE_HELP.map(([code, label]) => (
          ...
          >
            <p className="text-xs font-semibold" style={{ color: "#075E5D" }}>
              {label}
            </p>
            <code className="mt-1 block text-xs" style={{ color: "#64748B" }}>
              {shortcodeToken(code)}
            </code>
          </div>
        ))}
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ color: checked ? "#047857" : "#64748B" }}
      >
