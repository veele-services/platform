...
export function KnowledgebaseAutocompleteSearch({
  ...
  const visibleSuggestions = useMemo(
    () => suggestions.filter((suggestion) => suggestion.label.trim() || suggestion.value.trim()).slice(0, 10),
    [suggestions],
  );
    ...
    <form ref={rootRef} className={`relative flex gap-2 ${className}`} action="/help">
      <div className="relative min-w-0 flex-1">
        ...
        <input
          ...
          className="h-11 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3 text-sm shadow-sm focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100"
        />
        {open && visibleSuggestions.length > 0 && (
          <div
            id={listId}
            role="listbox"
            className="absolute left-0 right-0 top-12 z-40 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl"
          >
            {visibleSuggestions.map((suggestion, index) => (
              <button
                ...
                className={`flex min-h-12 w-full items-start gap-3 px-3 py-2 text-left text-sm transition ${
                  activeIndex === index ? "bg-cyan-50" : "bg-white hover:bg-slate-50"
                }`}
              >
                {suggestion.type === "article" ? (
                  <BookOpen className="mt-0.5 h-4 w-4 flex-shrink-0 text-cyan-700" />
                ) : (
                  <Tags className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-500" />
                )}
                <span className="min-w-0">
                  <span className="block truncate font-semibold text-slate-950">{suggestion.label}</span>
      ...
      <button
        type="submit"
        className="inline-flex h-11 items-center justify-center rounded-md bg-cyan-700 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-cyan-800"
      >
        {submitLabel}
      </button>
    </form>
