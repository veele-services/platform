...
function itemMeta(row: InventoryRow): string {
  return (
    [row.categoryName, row.type, row.brand, row.model]
      .filter(Boolean)
      .join(" - ") || "Geen categorie"
  );
}
...
export function InventoryView({
  ...
  function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    run(
      () => createInventoryItem(buildInput(formData)),
                    ...
                    <tr key={row.id} className="hover:bg-slate-50/70">
                      ...
                      <td className="px-4 py-3" style={{ color: "#475569" }}>
                        {row.nextInspectionDate
                          ? new Date(
                              `${row.nextInspectionDate}T00:00:00`,
    ...
    <Sheet>
      ...
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader>
          ...
          <SheetDescription>
            Registreer locatie, status, keuring en dossierinformatie.
          </SheetDescription>
        </SheetHeader>
        <form onSubmit={onSubmit} className="mt-6 flex flex-col gap-4">
          ...
          <p className="text-xs" style={{ color: "#64748B" }}>
            Code wordt automatisch als I000001 per tenant gegenereerd.
          </p>
        </form>
      </SheetContent>
    </Sheet>
