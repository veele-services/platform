...
export function MaterialsView({
  ...
  const [stockType, setStockType] = useState<MaterialStockMovementInput["movementType"]>("received");
  const [targetType, setTargetType] = useState("object");
  const [searchInput, setSearchInput] = useState(initialSearch);

  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const activeRows = useMemo(() => rows.filter((row) => !row.archivedAt && row.isActive), [rows]);
  ...
  function handleStock(event: FormEvent<HTMLFormElement>) {
    ...
    const form = event.currentTarget;
    const formData = new FormData(form);
    const movementType = (formString(formData, "movementType") ?? "received") as MaterialStockMovementInput["movementType"];
    const input: MaterialStockMovementInput = {
      ...
      notes: formString(formData, "notes"),
      fromStockLocationId: movementType === "transferred" ? formString(formData, "fromStockLocationId") : null,
      toStockLocationId: targetType === "existing" ? formString(formData, "toStockLocationId") : null,
      toObjectId: targetType === "object" ? formString(formData, "toObjectId") : null,
      toPersonnelId: targetType === "personnel" ? formString(formData, "toPersonnelId") : null,
    };

    run(() => recordMaterialStockMovement(input), "Voorraadmutatie opgeslagen.");
    form.reset();
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            onChange={(event) => applyFilters({ categoryId: event.target.value || undefined })}
          ...
          <select
            value={initialStatus}
            onChange={(event) => applyFilters({ status: event.target.value === "active" ? undefined : event.target.value })}
            className="h-10 rounded-md border bg-white px-3 text-sm"
            style={{ borderColor: "#CBD5E1", color: "#081D3A" }}
          >
            ...
            <table className="w-full min-w-[980px] text-sm">
              <thead style={{ backgroundColor: "#F8FAFC" }}>
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Materiaal</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Categorie</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Voorraad</th>
                    ...
                    <tr key={row.id} className="hover:bg-slate-50/70">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs" style={{ color: "#64748B" }}>
                          {row.unit}{row.defaultInvoiceable ? " - standaard factureerbaar" : ""}
                        </p>
                      </td>
                      <td className="px-4 py-3" style={{ color: "#475569" }}>{row.categoryName ?? "-"}</td>
                      <td className="px-4 py-3 text-right font-medium" style={{ color: row.negativeLocationsCount > 0 ? "#B91C1C" : "#081D3A" }}>
                        {formatNumber(row.totalStock)} {row.unit}
                        <p className="text-xs font-normal" style={{ color: "#94A3B8" }}>
                          {row.locationsCount} locatie{row.locationsCount === 1 ? "" : "s"}
                        </p>
                      </td>
                      <td className="px-4 py-3">
                        <span
                        ...
                        >
                          {status.icon && <AlertTriangle className="h-3.5 w-3.5" />}
                          {status.label}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-2">
                          ...
                          {canWrite && !row.archivedAt && (
                            <button
                              type="button"
                              onClick={() => run(() => archiveMaterial(row.id), "Materiaal gearchiveerd.")}
