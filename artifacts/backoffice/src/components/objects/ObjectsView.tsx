          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid #E2E8F0" }}>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Name</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Code</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Customer</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>Sector</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>City</th>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={canWrite ? 8 : 7} className="px-4 py-12 text-center text-sm" style={{ color: "#94A3B8" }}>
                    No objects found
                  </td>
                </tr>
              ) : (
                rows.map((row, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-medium text-sm" style={{ color: "#081D3A" }}>{row.name}</td>
                    <td className="px-4 py-3 text-sm" style={{ color: "#64748B" }}>{row.code ?? "—"}</td>
                    <td className="px-4 py-3 text-sm">
                      {row.customerName ? (
                        <Link href={`/customers/${row.customerId}`} className="hover:underline" style={{ color: "#00B7B3" }}>
                          {row.customerName}
                        </Link>
                      ) : "—"}
                    </td>
                    <td className="px-4 py-3 text-sm" style={{ color: "#64748B" }}>{row.sectorName ?? "—"}</td>
                    <td className="px-4 py-3 text-sm" style={{ color: "#64748B" }}>{row.city ?? "—"}</td>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="w-[520px] sm:max-w-[520px] overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>
              {editingId ? "Update object details below." : "Fill in the details to create a new object."}
            </SheetDescription>
          </SheetHeader>
          <ObjectForm
